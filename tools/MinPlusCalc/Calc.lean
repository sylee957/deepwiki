...
`(vals, period, incr)` triple into a `UppSeq ℚ` (checking `0 < period ≤ #vals`), then the CLI runs
...
/-- Validate a raw triple into a `UppSeq ℚ` (`0 < period ≤ #vals`). -/
def buildUpp (r : RawUpp) : Except String (UppSeq ℚ) :=
...
def sample (u : UppSeq ℚ) (k : Nat) : List ℚ := (List.range k).map u.evalNat

/-- Per-`d` increment (asymptotic slope over the common period `d = lcm`) of the first operand. -/
def slope (r s : UppSeq ℚ) : ℚ := ((Nat.lcm r.period s.period / r.period : Nat) : ℚ) * r.incr
...
def windowOk (r s : UppSeq ℚ) (N₀ : Nat) : Bool :=
...
def findCrossover (r s : UppSeq ℚ) (threshold fuel : Nat) : Nat :=
  match fuel with
  | 0 => threshold
...
def convUpp (r s : UppSeq ℚ) : UppSeq ℚ :=
...
def stableRank (r s : UppSeq ℚ) : Nat :=
  let tr := r.vals.length - r.period
  let ts := s.vals.length - s.period
...
`min_evalNat_add_lcm_window`. -/
def minUpp (r s : UppSeq ℚ) : UppSeq ℚ :=
...
def maxUpp (r s : UppSeq ℚ) : UppSeq ℚ :=
...
def tokenBucket (r b : ℚ) : UppSeq ℚ :=
...
example : ∀ n ∈ Finset.range 6,
    (tokenBucket 1 2).deconvNat (betaRL 2 1) n = (n : ℚ) + 3 := by native_decide
...
def backlogBound (α β : UppSeq ℚ) : ℚ := α.deconvNat β 0
...
def delayDominates (α β : UppSeq ℚ) (d : Nat) : Bool :=
...
def delayBound (α β : UppSeq ℚ) : Nat :=
  let fuel := (backlogBound α β).ceil.toNat + α.deconvBound β + 1
...
theorem delayBound_least (α β : UppSeq ℚ) {d : Nat}
    (hfind : (List.range (((backlogBound α β).ceil.toNat + α.deconvBound β + 1) + 1)).find?
...
def residualAt (β α : UppSeq ℚ) (n : Nat) : ℚ :=
...
theorem clampedDiff_le_residualAt (β α : UppSeq ℚ) {n m : Nat} (hm : m ≤ n) :
...
theorem residualAt_le (β α : UppSeq ℚ) {n : Nat} {x : ℚ}
...
theorem residualAt_nonneg (β α : UppSeq ℚ) (n : Nat) : 0 ≤ residualAt β α n :=
...
theorem residualAt_mono (β α : UppSeq ℚ) : Monotone (residualAt β α) := by
  intro a b hab
...
def residualSample (β α : UppSeq ℚ) (k : Nat) : List ℚ :=
...
/-- Lift an integer UPP sequence to `WithTop ℚ` — the sub-additive closure needs `δ₀`'s `⊤`. -/
def liftUpp (f : UppSeq ℚ) : UppSeq (WithTop ℚ) :=
  ⟨f.vals.map WithTop.some, (f.incr : WithTop ℚ), f.period, f.hperiod,
...
def closureSample (f : UppSeq ℚ) (k : Nat) : List (WithTop ℚ) :=
...
example : closureSample ⟨[0, 4], 1, 1, by decide, by decide⟩ 4 = [(0 : WithTop ℚ), 4, 5, 6] := by
  native_decide
