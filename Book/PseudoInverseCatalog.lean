...
theorem levelGeSet_rateE (R : ℝ≥0∞) (hR : 0 < R) (hRtop : R ≠ ⊤) (x : ℝ≥0∞) :
    levelGeSet (rateE R) x = Ici (R⁻¹ * x) := by
  ext t
  rw [mem_levelGeSet, mem_Ici, ge_iff_le, rateE, rateV,
    ENNReal.inv_mul_le_iff hR.ne' hRtop]

/-- `λ_R⁻¹ = λ_{1/R}`: the peak-rate inverse is peak-rate of reciprocal slope
(for `0 < R < ⊤`). -/
theorem pseudoInv_rateE (R : ℝ≥0∞) (hR : 0 < R) (hRtop : R ≠ ⊤) (x : ℝ≥0∞) :
    pseudoInv (rateE R) x = rateE R⁻¹ x := by
  rw [pseudoInv, levelGeSet_rateE R hR hRtop, csInf_Ici, rateE, rateV]

/-! ## Rate-latency: `β_{R,T}⁻¹ = γ_{1/R,T}` -/

/-- The admissible set `{t | x ≤ R·(t-T)}` is `[R⁻¹·x + T, ∞)` for `0 < x`,
`0 < R < ⊤`, `T < ⊤`. -/
theorem levelGeSet_rateLatencyE (R T : ℝ≥0∞) (hR : 0 < R) (hRtop : R ≠ ⊤)
    (hT : T ≠ ⊤) (x : ℝ≥0∞) (hx : 0 < x) :
    levelGeSet (rateLatencyE R T) x = Ici (R⁻¹ * x + T) := by
  ext t
  rw [mem_levelGeSet, mem_Ici, ge_iff_le, rateLatencyE, rateLatencyV,
    ← ENNReal.inv_mul_le_iff hR.ne' hRtop]
  constructor
  · intro h
    have hTt : T ≤ t := by
...
/-- `tokenBucketE r b x = r·x + b` for `0 < x` (the `delay 0` factor is `⊤`). -/
theorem tokenBucketE_pos (r b : ℝ≥0∞) {x : ℝ≥0∞} (hx : 0 < x) :
    tokenBucketE r b x = r * x + b := by
  rw [tokenBucketE, tokenBucketV]
...
theorem pseudoInv_rateLatencyE (R T : ℝ≥0∞) (hR : 0 < R) (hRtop : R ≠ ⊤)
    (hT : T ≠ ⊤) (x : ℝ≥0∞) (hx : 0 < x) :
    pseudoInv (rateLatencyE R T) x = tokenBucketE R⁻¹ T x := by
  rw [pseudoInv, levelGeSet_rateLatencyE R T hR hRtop hT x hx, csInf_Ici,
    tokenBucketE_pos R⁻¹ T hx]

/-! ## Affine / token-bucket: `γ_{r,b}⁻¹ = β_{1/r,b}` -/

/-- Pointwise form: `γ_{r,b} t = 0` at `t = 0`, else `r·t + b`. -/
theorem tokenBucketE_apply (r b t : ℝ≥0∞) :
    tokenBucketE r b t = if t = 0 then 0 else r * t + b := by
  rw [tokenBucketE, tokenBucketV]
  simp only [Pi.inf_apply, delay_apply]
  by_cases h : t = 0
  · subst h; simp
...
`rateLatencyE r⁻¹ b x = r⁻¹·(x - b)` even when `x ≤ b`. -/
theorem pseudoInv_tokenBucketE (r b : ℝ≥0∞) (hr : 0 < r) (hrtop : r ≠ ⊤)
    (x : ℝ≥0∞) (hx : 0 < x) :
    pseudoInv (tokenBucketE r b) x = rateLatencyE r⁻¹ b x := by
  show pseudoInv (tokenBucketE r b) x = r⁻¹ * (x - b)
  ...
  · -- `≤`: exhibit an admissible time at the claimed value.
    ...
    · -- `x ≤ b`: value is `0`; every positive time is admissible.
      ...
      intro a ha
      apply pseudoInv_le_of_le_apply
      rw [tokenBucketE_apply, if_neg (ne_of_gt ha)]
      exact le_trans hbx le_add_self
  · -- `≥`: every admissible time is `≥ r⁻¹·(x-b)`.
    apply le_pseudoInv
    intro t ht
    rw [tokenBucketE_apply] at ht
    have htne : t ≠ 0 := by
      rintro rfl
      rw [if_pos rfl] at ht
...
/-- Degenerate rate `r = 0`: `γ_{0,b}⁻¹ = δ_b`. The token-bucket `tokenBucketE 0 b`
...
theorem pseudoInv_tokenBucketE_zero_rate (b x : ℝ≥0∞) :
    pseudoInv (tokenBucketE 0 b) x = delayE b x := by
  rw [delayE, delay_apply]
  by_cases hxb : x ≤ b
  · -- `x ≤ b`: reached at every positive time, so the first crossing is `0`.
    ...
    apply pseudoInv_le_of_le_apply
    rw [tokenBucketE_apply, if_neg ha0, zero_mul, zero_add]
    exact hxb
  · -- `x > b`: never reached, so the admissible set is empty and the inf is `⊤`.
    rw [if_neg hxb, pseudoInv,
      show levelGeSet (tokenBucketE 0 b) x = ∅ from ?_, sInf_empty]
    ext t
...
The delay inverts to the rate-`0`, latency-`d` token-bucket `tokenBucketE 0 d`:
the constant `d` for `x > 0`, and `0` at `x = 0` (the dual of
`pseudoInv_tokenBucketE_zero_rate`). The two branches are `pseudoInv_delayE_pos`
and `pseudoInv_delayE_zero` in `PseudoInverse`. -/

/-- `δ_d⁻¹ = γ_{0,d}`: the delay inverts to the rate-`0` token-bucket
`tokenBucketE 0 d`, for every `x`. -/
theorem pseudoInv_delayE_eq_tokenBucket (d x : ℝ≥0∞) :
    pseudoInv (delayE d) x = tokenBucketE 0 d x := by
  rw [tokenBucketE_apply, zero_mul, zero_add]
  rcases eq_or_ne x 0 with hx | hx
  · subst hx; rw [if_pos rfl]; exact pseudoInv_delayE_zero d
  · rw [if_neg hx]; exact pseudoInv_delayE_pos d (pos_iff_ne_zero.mpr hx)
