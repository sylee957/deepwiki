...
* `DeepWiki.SymbolicIntegration.Engine.Algebraic.RadicalLogArgGenericExamples` — **computes the log
  ...
  (`gaussElim`/`kernelVector`/`radLogArgSolve`), the whole linear solve running over the tower field
...
* `DeepWiki.SymbolicIntegration.Engine.ElementaryIntegrate` — the **unified integrator**
  `cIntegrateElementary` assembling `v + Σ cᵢ log uᵢ` (output `AlgIntegralResult`, differentiated by the
  ACTUAL-tower-derivation `algDeriv`); round-trip `∫√(eˣ+1) dx = 2√(eˣ+1) + log((y−1)/(y+1))`.
...
log-derivative `β`-linear system `radDeriv(N)·D − N·D' − radMul(N,integrand)·D = 0` over `α = DenseFrac β`,
...
abbrev bie_logarg_over_tower := @radLogArgSolve

/-- **★ `radLogArgSolve` COMPUTES `u = (y−1)/(y+1)` for `∫ dx/√(eˣ+1)` over ℚ(x)(eˣ)** (Bronstein 1990,
...
arbitrary base field `α` (the tower level `DenseFrac β`). The generic analogue of `AlgIntegralResult (DenseFrac ℚ)`; the
...
integral `v + Σ log u` over a transcendental tower): `algDeriv ρ F = radDeriv v + Σ cᵢ · radLogDeriv uᵢ`
...
derivative (`θ' = θ`); the round-trip compares `algDeriv ρ (cIntegrateElementary …)` to the integrand. -/
abbrev bie_unified_deriv := @algDeriv
...
`α = DenseFrac β`. Given the log-solve data, it **COMPUTES** the log argument on the residual via
...
the ACTUAL exp-tower derivative `algDeriv F = y` (the integrand `√(eˣ+1)`); fed back to
`cIntegrateElementary`, the engine COMPUTES the log half (`radLogArgSolve → N = (θ+2)−2y`, `u = N/θ`),
assembles `F' = ⟨2y, [(1, u)]⟩`, and `algDeriv F' = y` — BOTH halves, over the tower, through the real
...
derivation), the log argument `u = (y−1)/(y+1)` COMPUTED by `radLogArgSolve`, and `algDeriv F' = y` (the
