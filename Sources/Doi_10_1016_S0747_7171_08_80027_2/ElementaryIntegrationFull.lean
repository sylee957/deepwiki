...
abbrev bie_case3_cofactor_tower := @CPolyG.radCase3CofactorTower

/-- **One Case-3 degree-lowering step over a tower** (Bronstein 1990, elementary integral `v + Σ log u` over
a transcendental tower, rational part COMPUTED): `radReduceCase3IterateG` is the Case-3 iterator with the
cofactor swapped to `radCase3CofactorTower der`, so the **ACTUAL** derivation drives both the cofactor `B`
and the residual `D = der B·f + Bg − C`. One structural step per fuel, accumulating `B·f` into the
rational-part numerator, bottoming at `deg C < deg f`. -/
abbrev bie_case3_iterate_tower := @CPolyG.radReduceCase3IterateG
...
the ACTUAL derivation `der` (e.g. `cmonomialDeriv [θ]`, `θ' = θ` over the exp tower) to **compute** the
rational-part numerator. Over `α = ℚ(x)(eˣ)` for `∫√(eˣ+1) dx` it computes `vNum = 2ρ`, so the rational
part `v = 2y = 2√(eˣ+1)` — an OUTPUT, no longer a supplied constant. -/
abbrev bie_rational_over_tower := @CPolyG.radIntegrateCase3G
