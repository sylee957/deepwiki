...
quadratic-Hermite engine `hermiteReduce` over `CPoly ℚ := List ℚ` runs on
...
recursion over a dense coefficient list `CPoly ℚ := List ℚ` (Mathlib's `ℚ[X]` arithmetic is
...
bounded for termination, returning the arctan arguments as `(numerator, denominator)` `CPoly ℚ` pairs.
The library's `logToAtanCompute`, with the `toPoly : CPoly ℚ → ℚ[X]` bridge and its homomorphism lemmas
...
rendering of `R(t) = res_x(D, A − t·D')` over the dense coefficient carrier `CPoly ℚ := List ℚ` (Mathlib's
...
`∫ A/D = ∑_{R(a)=0} a·log(S(a,x))`, over the bivariate carrier `BPoly := List (CPoly ℚ)` (`= ℚ[t][x]`;
