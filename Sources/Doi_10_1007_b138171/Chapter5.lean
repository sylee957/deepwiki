...
canonical **generic** engine at `α = QFunNZ ℚ` (the recursive `Frac(ℚ[x])`, every instance bottoming at
...
level and instantiable at `QFunNZ ℚ` like the rest of the engine.
...
The §5 smoke examples over the generic ℚ(x) = `QFunNZ ℚ` carrier read their ℚ(x) coefficients as
...
def qConst5 (n : ℚ) : QFunNZ ℚ := ⟨([n], [(1 : ℚ)]), QFunNZ.cisZeroG_one_singleton⟩

/-- A ℚ(x) fraction `num/den` as a `QFunNZ ℚ` element, with `den ≠ 0` discharged by `native_decide`. -/
def qFrac5 (num den : List ℚ) (h : CPoly.cisZero den = false := by native_decide) : QFunNZ ℚ :=
  ⟨(num, den), h⟩
...
engine, here at the generic ℚ(x) = `QFunNZ ℚ`) over the tower ℚ(x)[t], rewriting the normal part `f = a/d`
...
noncomputable abbrev alg_5_3_hermiteReduce := cHermiteReduceTower (α := QFunNZ ℚ)
...
theorem ex_5_3_1 :
    (let Dt : CPoly (QFunNZ ℚ) := [qConst5 1, qConst5 0, qConst5 1]      -- `Dt = t²+1`
     let a : CPoly (QFunNZ ℚ) := [qConst5 1]                           -- `a = 1`
     let d : CPoly (QFunNZ ℚ) := [qConst5 0, qConst5 0, qConst5 1]       -- `d = t²`
...
`…G`-suffixed mirror is needed; here pinned at the generic ℚ(x) = `QFunNZ ℚ` like the rest of the
engine.)* -/
noncomputable abbrev alg_5_4_polynomialReduce := @cPolyReduceTower (QFunNZ ℚ)
...
theorem ex_5_4_1 :
    (let Dt : CPoly (QFunNZ ℚ) := [qConst5 1, qConst5 0, qConst5 1]       -- `Dt = t²+1`
     let p : CPoly (QFunNZ ℚ) := [qConst5 0, qConst5 0, qConst5 0, qConst5 1]  -- `p = t³`
...
engine, here at the generic ℚ(x) = `QFunNZ ℚ`) over the tower, by the evaluation + Lagrange-
...
noncomputable abbrev alg_5_6_residueResultant := cResidueResultantTower (α := QFunNZ ℚ)
...
`QFunNZ ℚ`) over the tower — the polynomial inside `log` for a residue `c`, so
`∑_c c·log(cLogArgTower … c)` is the logarithmic part of `∫ a/d`. Computable +
`native_decide`-validated; abstract correctness deferred. -/
noncomputable abbrev alg_5_6_logArg := cLogArgTower (α := QFunNZ ℚ)
...
theorem ex_5_6_2 :
    (let Dt : CPoly (QFunNZ ℚ) := [qFrac5 [1] [0, 1]]                       -- `Dt = 1/x`
     let a : CPoly (QFunNZ ℚ) := [qFrac5 [0, 0, -1] [1], qConst5 (-1), qConst5 2]  -- `a = 2t²−t−x²`
     let d : CPoly (QFunNZ ℚ) := [qConst5 0, qFrac5 [0, 0, -1] [1], qConst5 0, qConst5 1]  -- `d = t³−x²t`
     let resMonic : CPoly (QFunNZ ℚ) :=                                    -- `z³−xz²−z/4+x/4`
       [qFrac5 [0, 1] [4], qConst5 (-1/4), qFrac5 [0, -1] [1], qConst5 1]
     let argPlus : CPoly (QFunNZ ℚ) := [qFrac5 [0, 1] [1], qConst5 1]        -- `t + x`
     let argMinus : CPoly (QFunNZ ℚ) := [qFrac5 [0, -1] [1], qConst5 1]      -- `t − x`
...
`…G`-suffixed mirror is needed; here pinned at the generic ℚ(x) = `QFunNZ ℚ` like the rest of the
engine.)* -/
noncomputable abbrev alg_5_8_primitivePolyIntegrate := @cPrimitivePolyIntegrate (QFunNZ ℚ)
...
theorem ex_5_8_primitive :
    (let Dt : CPoly (QFunNZ ℚ) := [qFrac5 [1] [0, 1]]                       -- `Dt = 1/x`
     let p : CPoly (QFunNZ ℚ) := [qConst5 0, qConst5 0, qFrac5 [1] [0, 1]]  -- `p = (1/x)·t²`
