...
def qFrac5 (num den : List ℚ) (h : CPoly.cisZero den = false := by native_decide) : QFunNZG ℚ :=
  ⟨(num, den), h⟩
...
noncomputable abbrev alg_5_3_hermiteReduce := cHermiteReduceTower (α := QFunNZG ℚ)
...
noncomputable abbrev alg_5_6_residueResultant := cResidueResultantTower (α := QFunNZG ℚ)
...
noncomputable abbrev alg_5_6_logArg := cLogArgTower (α := QFunNZG ℚ)
