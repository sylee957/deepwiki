...
`α = CFrac ℚ` (the recursive `Frac(ℚ[x])`, every instance bottoming at ℚ with no hand-built piece). The
leading-coefficient base solve of the §6.6 cancellation cases is the **generic**
`CRischField.crischDESolve` over `CFrac ℚ` (the recursive tower oracle, every instance bottoming at ℚ).
...
num/den lists over `DensePoly ℚ = List ℚ`. `CFrac.ofScalar` embeds constants, while `qFrac6` packages
...
def qX6 : CFrac ℚ := qFrac6 [0, 1] [1]

/-- **Cleared Risch-DE identity check over the generic ℚ(x)[t]** (the `CFrac ℚ` mirror of the engine's
...
`cWeakNormalizer Dt fnum fden = q ∈ k[t]` (the generic engine at the generic ℚ(x) = `CFrac ℚ`)
...
noncomputable abbrev alg_6_1_weakNormalizer := cWeakNormalizer (α := CFrac ℚ)

/-- **Algorithm `RdeNormalDenominator`** (§6.2 eq. 6.2 / Cor 6.1.1, p.185): the fuel-free computable
`cRdeNormalDenominator Dt fnum fden gnum gden` (the generic engine at `CFrac ℚ`) over the tower,
...
noncomputable abbrev alg_6_2_normalDenominator := cRdeNormalDenominator (α := CFrac ℚ)
...
(`native_decide`). -/
theorem ex_6_1_2 :
    (let Dt : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1, CFrac.ofScalar 0, CFrac.ofScalar 1]              -- `Dt = t²+1`
     let fnum : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1, CFrac.ofScalar 0, CFrac.ofScalar 1]            -- `f = t²+1`
     let fden : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1]
     let gnum : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1]                                -- `g = 1/t²`
     let gden : DensePoly (CFrac ℚ) := [CFrac.ofScalar 0, CFrac.ofScalar 0, CFrac.ofScalar 1]
     let exA : DensePoly (CFrac ℚ) := [CFrac.ofScalar 0, CFrac.ofScalar 1]                       -- `a = t`
     let exB : DensePoly (CFrac ℚ) := [CFrac.ofScalar (-1), CFrac.ofScalar 1, CFrac.ofScalar (-1), CFrac.ofScalar 1]  -- `(t−1)(t²+1)`
     let exC : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1]                                 -- `c = 1`
     let exH : DensePoly (CFrac ℚ) := [CFrac.ofScalar 0, CFrac.ofScalar 1]                       -- `h = t`
...
fuel-free computable `cRdeSpecialDenominator Dt a b c` (the generic engine at `CFrac ℚ`) over the tower,
...
correctness deferred. -/
noncomputable abbrev alg_6_2_specialDenominator := cRdeSpecialDenominator (α := CFrac ℚ)
...
ℚ(x)[t] (`native_decide`). -/
theorem ex_6_2_2 :
    (let Dt : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1, CFrac.ofScalar 0, CFrac.ofScalar 1]
     let exA : DensePoly (CFrac ℚ) := [CFrac.ofScalar 0, CFrac.ofScalar 1]                       -- `a = t`
     let exB : DensePoly (CFrac ℚ) := [CFrac.ofScalar (-1), CFrac.ofScalar 1, CFrac.ofScalar (-1), CFrac.ofScalar 1]  -- `(t−1)(t²+1)`
     let exC : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1]                                 -- `c = 1`
...
p.198–201): the computable `cRdeBoundDegree Dt a b c = n ∈ ℕ` (the generic engine at `CFrac ℚ`)
...
theorem ex_6_3_4 :
    (let Dt : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1, CFrac.ofScalar 0, CFrac.ofScalar 1]
     let exA : DensePoly (CFrac ℚ) := [CFrac.ofScalar 0, CFrac.ofScalar 1]
     let exB : DensePoly (CFrac ℚ) := [CFrac.ofScalar (-1), CFrac.ofScalar 1, CFrac.ofScalar (-1), CFrac.ofScalar 1]
     let exC : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1]
...
`cSPDE Dt a b c n` (the generic engine at `CFrac ℚ`) over the tower — the recursive
...
noncomputable abbrev alg_6_4_spde := cSPDE (α := CFrac ℚ)
...
(the generic engine at `CFrac ℚ`) routing `Dq + b·q = c` to the non-cancellation solver or the
primitive/hyperexponential cancellation solvers by monomial type and `deg(b)` (Lemma 6.5.1). -/
noncomputable abbrev alg_6_5_polyRischDE := cPolyRischDE (α := CFrac ℚ)

/-- **The full Risch DE solver**: the fuel-free computable `cRischDE Dt fnum fden gnum gden` (the
canonical generic engine, here at the generic ℚ(x) = `CFrac ℚ`) over the tower, chaining normal
...
noncomputable abbrev alg_6_rischDE := cRischDE (α := CFrac ℚ)
...
theorem ex_6_4_1 :
    (let Dt : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1, CFrac.ofScalar 0, CFrac.ofScalar 1]
     let fnum : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1, CFrac.ofScalar 0, CFrac.ofScalar 1]
     let fden : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1]
     let gnum : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1]
     let gden : DensePoly (CFrac ℚ) := [CFrac.ofScalar 0, CFrac.ofScalar 0, CFrac.ofScalar 1]            -- `1/t²`
...
`CRischField.crischDESolve (α := CFrac ℚ) f g = Option (CFrac ℚ)` solving `Ds + f·s = g` over
`k = ℚ(x)` (`D = d/dx`) — the leading-coefficient recursion target of the cancellation cases. The
recursive `CRischField (CFrac ℚ)` instance runs the generic §6 pipeline over `ℚ[x]` and bottoms at the
`CRischField ℚ` constant solve. -/
noncomputable abbrev alg_6_6_rischDEBase := @CRischField.crischDESolve (CFrac ℚ) _ _
...
theorem ex_6_6_cancelPrim :
    (let Dt : DensePoly (CFrac ℚ) := [qFrac6 [1] [0, 1]]                          -- `Dt = 1/x`
     let b : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1]                                   -- `b = 1`
     let c : DensePoly (CFrac ℚ) := [qFrac6 [1] [0, 1], CFrac.ofScalar 1]                 -- `c = t + 1/x`
...
theorem ex_6_6_cancelExp :
    (let Dt : DensePoly (CFrac ℚ) := [CFrac.ofScalar 0, CFrac.ofScalar 1]                        -- `Dt = t`
     let b : DensePoly (CFrac ℚ) := [qFrac6 [1] [0, 1]]                           -- `b = 1/x`
     let c : DensePoly (CFrac ℚ) := [CFrac.ofScalar 0, qFrac6 [2, 1] [1]]                 -- `c = (2+x)t`
...
theorem ex_6_6_baseRecursion :
    (let Dt : DensePoly (CFrac ℚ) := [qFrac6 [1] [0, 1]]                          -- `Dt = 1/x`
     ...
     let fden : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1]
     let gnum : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1, CFrac.ofScalar 2]                      -- `g = 2t + 1`
     let gden : DensePoly (CFrac ℚ) := [CFrac.ofScalar 1]
     (match DensePoly.cRischDE Dt fnum fden gnum gden with
       | some (ynum, yden) => rdeClearedCheckG6 Dt fnum fden gnum gden ynum yden
       | none => false)
     && (match CRischField.crischDESolve (qFrac6 [1] [0, 1]) (CFrac.ofScalar 2) with
...
`CRischField.crischDESolve (α := CFrac ℚ)` on `Ds + (1/x)s = 2` returns `s = x` — the whole Ch. 6
