...
/-- **The minimal polynomial** `q(y) = y⁴+2y²−2` of `θ = √(−1+√3)` as a `DensePoly ℚ` (in `y`), low→high:
...
-- **The RT resultant `R(t) ∈ K[t]`** (degree 5; coefficients in `K = ℚ(θ)`, each a `DensePoly ℚ`-in-`θ`).
