...
tower (`DeepWiki.SymbolicIntegration.Engine.Parametric`), each with `native_decide` evidence on a
worked book example.

**Computable-vs-abstract.** Each algorithm below is a computable function (the parametric solve
reducing to a constant linear system over `Const(k) = ℚ` via `cNullspaceBasisQ`), validated by
`native_decide` on the book's example (matching the book's constraint matrix / kernel, or checking the
...
§7.1 The Parametric Risch Differential Equation: Thm 7.1.1, Thm 7.1.2; Cor 7.1.1; Lemma 7.1.1,
  ...
  native_decide-validated on Ex 7.1.1, see `alg_7_1_paramRischDE`/`ex_7_1_1`). The genuine-tower case
...
§7.2 The Limited Integration Problem: Thm 7.2.1; Cor 7.2.1; Ex 7.2.1, Ex 7.2.2 (abstract correctness +
  ...
  native_decide-validated as the `gᵢ = Dwᵢ/wᵢ` specialization of `ParamRDE`, see
...
§7.3 The Parametric Logarithmic Derivative Problem: Lemma 7.3.1; Ex 7.3.1 (abstract correctness + the
  ...
  `ParametricLogarithmicDerivative` is now computable + native_decide-validated on Ex 7.3.2, see
...
the computable `cParamRischDE fuel gnums gdens` over the base monomial ℚ[t] (`D = d/dt`,
...
linear solve `cNullspaceBasisQ` (`ConstantSystem`, Lemma 7.1.2). Computable + `native_decide`-validated;
abstract correctness deferred. -/
def alg_7_1_paramRischDE := @cParamRischDE

/-- **Example 7.1.1** (§7.1, the `LinearConstraints`/`ConstantSystem` boxes, p.223/224): for
`Dp = c₁·(2t³+3t+1)/(t²−1) + c₂/(t−1) + c₃/(t+1)`, `cLinearConstraintsQ` returns the homogeneous matrix
`[[5,1,1],[1,1,-1]]` (eq. 7.10) and `cParamRischDE` returns a one-vector kernel basis proportional to
`(1,−3,−2)`, each verified to satisfy the eq. 7.6 constraint (`native_decide`). -/
...
the computable `cLimitedIntegrate fuel fnum fden wnums wdens` over the base monomial ℚ[t], deciding
`f = Dv + Σᵢ cᵢ·log(wᵢ)` as the `gᵢ = Dwᵢ/wᵢ` specialization of `cParamRischDE` (with `f` the forced
generator), returning the basis of admissible constant tuples. Computable + `native_decide`-validated;
abstract correctness and the Cor 7.2.1 back-substitution deferred. -/
def alg_7_2_limitedIntegrate := @cLimitedIntegrate

/-- **Example (§7.2, p.245)**: for `f = 1/t`, `w₁ = t`, `w₂ = t+1`, `cLimitedIntegrate` returns a
nonempty constant kernel basis, each satisfying the eq. 7.6 constraint, finding the relation
`f = log(t)` — the limited-integral certificate that `∫ f = log(t)` (`native_decide`). -/
...
(the §5.12 construction) is the documented continuation. Computable + `native_decide`-validated;
abstract correctness deferred. -/
