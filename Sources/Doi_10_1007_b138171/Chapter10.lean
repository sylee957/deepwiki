...
(`DeepWiki.SymbolicIntegration.ComputableParallel`): Yun squarefree factorization (the candidate log
...
**Computable-vs-abstract.** Each algorithm below is a computable function over `CPolyG ℚ` (= ℚ(t))
...
computable `cParallelIntegrateTower fuel Dt a d` over `a d : CPolyG QFunNZ`. The base-field case (`Dt, a,
d` all with `ℚ`-constant coefficients, so `k = ℚ`) is routed through `cParallelIntegrate` and lifted back
to `QFunNZ` coefficients; a genuine `x`-dependent coefficient (the full tower, needing the §10.2
