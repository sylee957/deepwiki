...
/-- **`A = 72x⁷+256x⁶−192x⁵−1280x⁴−312x³+1440x²+576x−96`** as a `DensePoly ℚ` (Exercise 2.3 numerator),
coefficients low→high. -/
def cA23 : DensePoly ℚ := [-96, 576, 1440, -312, -1280, -192, 256, 72]

/-- **`D = 9x⁸+36x⁷−32x⁶−252x⁵−78x⁴+468x³+288x²−108x+9`** as a `DensePoly ℚ` (Exercise 2.3 denominator),
coefficients low→high. -/
def cD23 : DensePoly ℚ := [9, -108, 288, 468, -78, -252, -32, 36, 9]
...
(`× 35` to clear the interpolation denominators): a **degree-8** integer polynomial as a `DensePoly ℚ`,
whose eight roots are the residues of `A/D`. -/
def cR23full : DensePoly ℚ :=
...
def cR23 : DensePoly ℚ := csqfreePart 80 (rtResultantCompute 80 cA23 cD23)
...
def cS1_23_lower : DensePoly ℚ := cnorm (bevalX (-2) cS1_23)
...
-- `logToAtanCompute_ex281`) runs on the same `DensePoly ℚ` carrier the LRT output lives on. For Example
