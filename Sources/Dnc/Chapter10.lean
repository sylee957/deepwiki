...
/-! **Definition 10.1** (§10.2, p.233): Network topology classes: feed-forward (acyclic flow graph, topological sort), tandem (servers in a line), and nested tandem (flow paths totally ordered by inclusion, (Nest): i<j ⇔ pᵢ⊆pⱼ). Library: concatComp, aggregateServer, ServersResidualPmooPath.pathHops. -/
...
/-! **Lemma 10.1** (§10.3.2, p.237): Cumulative-process inequality underlying Theorem 10.1: ∀t ∃u₁..uₙ≥0 with ∑uⱼ such that F₁⁽ⁿ⁾(t)−F₁⁽⁰⁾(t−∑uⱼ) ≥ ∑ⱼβⱼ(uⱼ) − ∑ᵢ(Fᵢ^{end(i)}(…)−Fᵢ^{(0)}(…)), and moreover F₁⁽ⁿ⁾(t)−F₁⁽⁰⁾(t−∑uⱼ)≥0 — the telescope of per-hop strict steps over the cascaded starts. Library: sum_add_pathTelescope_le, pathNode_strict_step, sum_add_chainConv_le_of_strict_chain. -/
...
/-! **Example 10.3** (§10.3.4.2, p.242): FIFO-PMOO worked on the sink-tree of Figure 10.1 with rate-latency servers and token-bucket arrivals, optimizing θ₁ (greater than C₁) to compute the per-path delay bound d. Not formalized in the library. -/
...
/-! **Example 10.4** (§10.4.1, p.244): TOA worked on Figure 10.2 (top): the output arrival curves αᵢ⁽²⁾ are computed as αᵢ⁽¹⁾⊘β⁽¹⁾ and combined, then the worst-case backlogged-period bound per server. Not formalized in the library. -/
...
/-! **Example 10.5** (§10.4.2, p.247): SFA worked on the table of Figure 10.1 (top): explicit per-server residual and output arrival curves, then the path convolution giving each flow's end-to-end service curve. Not formalized in the library. -/
...
/-! **Example 10.6** (§10.4.3, p.249): GFA worked on Figure 10.6 with flow 2 the flow of interest: arc-wise grouping of cross-traffic, the grouped arrival/service curves η and β̃ at each server. Not formalized in the library. -/
