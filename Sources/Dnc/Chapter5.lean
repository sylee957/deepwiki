...
/-! **Proposition 5.2** (§5.1, p.102): Properties of maximal arrival curves: α* and its sub-additive closure are maximal arrival curves, any α'≥α maximal is maximal, A⊘A is a (least) maximal arrival curve, and the left-continuous extension is one too. Library: DeepWiki.isMaximalArrivalBound_minDeconv_self, DeepWiki.minDeconv_self_le_of_isMaximalArrivalBound, DeepWiki.IsMaximalArrivalBound.mono, DeepWiki.IsMaximalArrivalCurve.subadditiveClosureENN, DeepWiki.IsMaximalArrivalBound.subadditiveClosureMin, DeepWiki.isMaximalArrivalBound_leftLim_of_isLeftContinuous. -/

/-! **Corollary 5.1** (§5.1, p.103): If A admits a maximal (resp. minimal) arrival curve it admits a non-decreasing left- (resp. right-) continuous one: the left/right-limit extension is again an arrival curve. Library: DeepWiki.isMaximalArrivalBound_leftLim_of_isLeftContinuous, DeepWiki.isMaximalArrivalBound_leftLim_of_isRightContinuous, DeepWiki.isMinimalArrivalBound_rightLim_of_isRightContinuous, DeepWiki.isMinimalArrivalBound_rightLim_of_isLeftContinuous. -/

/-! **Proposition 5.3** (§5.1, p.104): Properties of minimal arrival curves (dual of Prop 5.2): α'* and the super-additive closure are minimal arrival curves, any α''≤α' is minimal, A⊘̄A is a (greatest) minimal arrival curve, and the right-continuous extension is one too. Library: DeepWiki.isMinimalArrivalBound_maxDeconv_self, DeepWiki.le_maxDeconv_self_of_isMinimalArrivalBound, DeepWiki.IsMinimalArrivalBound.mono, DeepWiki.IsMinimalArrivalBound.superadditiveClosureMax, DeepWiki.isMinimalArrivalBound_rightLim_of_isRightContinuous. -/

/-! **Theorem 5.1** (§5.1, p.105): Combining a minimal αˡ and maximal αᵘ arrival curve: ηᵘ=αᵘ⊘̄αˡ refines the maximal and ηˡ=αˡ⊘αᵘ the minimal curve; under sub- /super-additivity with vanishing at 0 the refinement fixes in one step and the sub- /super-additive closures are again arrival curves. Library: DeepWiki.isMaximalArrivalBound_etaMax, DeepWiki.isMinimalArrivalBound_etaMin, DeepWiki.etaMax_fixpoint, DeepWiki.etaMin_fixpoint, DeepWiki.isSubadditive_etaMax, DeepWiki.isSuperadditive_etaMin, DeepWiki.refineStep_eta_fixpoint. -/
...
/-! **Proposition 5.5** (§5.2.2, p.108): Backlogged-period properties: any subinterval of a backlogged period is backlogged; (Start(t),t] is backlogged; A(Start(t))=D(Start(t)); and Start is constant on a backlogged period. Library: DeepWiki.IsBacklogged.subset, DeepWiki.isBacklogged_Ioc_start, DeepWiki.apply_start_eq, DeepWiki.start_const_of_backlogged. -/
...
/-! **Proposition 5.6** (§5.2.2, p.109): Properties of strict service curves: if S offers strict β then it offers any strict β'≤β; if it offers β₁ and β₂ it offers β₁∨β₂; and S_strict(β)=S_strict(β↗)=S_strict(β*) (non-decreasing and super-additive closures). Library: DeepWiki.IsStrictMinimalServiceCurve.mono, DeepWiki.IsStrictMinimalServiceCurve.sup, DeepWiki.isStrictMinimalServiceCurve_closures_iff. -/
...
/-! **Proposition 5.11** (§5.2.4, p.114): A σ-greedy shaper is a σ-shaper and offers σ as both minimal and maximal min-plus service curve: S_grsh(σ)⊆S_minp(σ)∩S_sh(σ). Library: DeepWiki.greedyShaperRel_eq_minimalServiceRel_inf_shaperRel, DeepWiki.greedyShaperRel_le_shaperRel, DeepWiki.IsGreedyShaper.isMinimalServiceCurve, DeepWiki.IsGreedyShaper.isMaximalServiceCurve, DeepWiki.IsGreedyShaper.isShaper. -/

/-! **Theorem 5.2** (§5.3.1, p.115): Backlog and delay bounds: with α a maximal arrival curve for A and S offering min-plus minimal service β, d(A,D)≤hDev(α,β) and b(A,D)≤vDev(α,β); if α sub-additive and β left-continuous the bounds are tight (eq 5.21-5.23). Library: DeepWiki.delay_le_hDev_of_isMinimalServiceCurve, DeepWiki.backlog_le_vDev_of_isMinimalServiceCurve, DeepWiki.delay_eq_hDev_of_minConv_eq, DeepWiki.backlog_eq_vDev_of_minConv_eq, DeepWiki.exists_delay_eq_hDev_backlog_eq_vDev. -/

/-! **Proposition 5.12** (§5.3.1, p.115): Monotony of deviations: for f≤f', g≥g', vDev(f,g)≥vDev(f',g') and hDev(f,g)≥hDev(f',g'). Library: DeepWiki.vDev_mono, DeepWiki.hDev_mono, DeepWiki.vDevAt_mono, DeepWiki.hDevAt_mono. -/
...
/-! **Corollary 5.2** (§5.3.2, p.118): Tighter constraints give better bounds: if α'≤α and β'≥β then hDev(α',β')≤hDev(α,β) and vDev(α',β')≤vDev(α,β). Library: DeepWiki.hDev_mono, DeepWiki.vDev_mono. -/

/-! **Theorem 5.3** (§5.3.2, p.118): Output arrival curves: with min/max service βᵐ,βᴹ, shaper σ, and αᵘ,αˡ arrival curves for A, D has maximal arrival curve ηᵘ=((αᵘ∗βᴹ)⊘βᵐ)∧σ (eq 5.26) and minimal ηˡ=αˡ∗(βᵐ⊘̄βᴹ) (eq 5.27). Library: DeepWiki.isMaximalArrivalBound_output, DeepWiki.isMinimalArrivalBound_output, DeepWiki.isMaximalArrivalCurve_output, DeepWiki.isMinimalArrivalCurve_output. -/
...
/-! **Corollary 5.4** (§5.3.2, p.119): Output arrival curves for a σ-greedy shaper (σ sub-additive): D has maximal arrival curve αᵘ∗σ and minimal arrival curve αˡ∗(σ⊘̄σ). Library: DeepWiki.isMaximalArrivalBound_output_of_isGreedyShaper, DeepWiki.isMinimalArrivalBound_output_of_isGreedyShaper, DeepWiki.isMaximalArrivalCurve_output_of_isGreedyShaper, DeepWiki.isMinimalArrivalCurve_output_of_isGreedyShaper. -/
...
/-! **Proposition 5.13** (§5.3.3.1, p.120): Restricting the deviation domain to a finite interval: with maximal arrival curve α and strict service β, b(A,D)≤sup over [0,ℓmax] vDev(α,β,t) and d(A,D)≤sup over [0,ℓmax] hDev(α,β,t), ℓmax=inf{t>0|α(t)≤β(t)} (book prints inf, proof gives sup). Library: DeepWiki.backlog_le_biSup_vDevAt_sInf_of_isMinimalServiceCurve, DeepWiki.delay_le_biSup_hDevAt_sInf_of_isMinimalServiceCurve. -/

/-! **Theorem 5.5** (§5.3.3.1, p.122): Maximal length of a backlogged period: with A maximal arrival curve α and S strict service β, every backlogged period has length ℓmax=inf{d>0|α(d)≤β(d)} (eq 5.28). Library: DeepWiki.length_le_firstCrossing_of_isBacklogged, DeepWiki.maxBackloggedLength_le_firstCrossing. -/
...
/-! **Proposition 5.14** (§5.3.3.2, p.123): Deviations are deconvolutions: vDev(f,g)=(f⊘g)(0) (eq 5.30) and hDev(f,g)=((g⁻¹∘f)⊘λ₁)(0) (eq 5.31). Library: DeepWiki.vDev_eq_deconv_zero, DeepWiki.hDev_eq_deconv_pseudoInv_zero. -/

/-! **Theorem 5.6** (§5.3.3.3, p.123): Performance operators are continuity insensitive: for monotone left- /right-continuous f,g, hDev(fℓ,gℓ)=hDev(fᵣ,gᵣ) and vDev(fℓ,gℓ)=vDev(fᵣ,gᵣ). Library: DeepWiki.hDev_leftLim_eq_hDev_rightLim, DeepWiki.vDev_leftLim_eq_vDev_rightLim. -/
...
/-! **Lemma 5.4** (§5.3.3.3, p.124): Horizontal deviation insensitive to left/right closure of g: hDev(f,g,t)=hDev(f,gℓ,t)=hDev(f,gᵣ,t) (eq 5.32). Library: DeepWiki.hDevAt_leftLim_eq_hDevAt, DeepWiki.hDevAt_rightLim_eq_hDevAt. -/
