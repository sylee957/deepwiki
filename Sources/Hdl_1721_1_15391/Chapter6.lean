...
Ch. 5 §3 → integrator: wiring `(c/m, g)` into the non-principal branch — `principalGenerator` returns the
  generator `g`; packaging `(c/m, g)` as an `AlgIntegralResult (DenseFrac ℚ)` log term (the `cIntegrateAlgebraicWf` branch
...
DeepWiki.SymbolicIntegration.DensePoly`, referenced here as `DensePoly.<name>`. -/

/-- **Cantor composition** `DensePoly.cantorCompose ρ D₁ D₂` (Trager, Chapter 6 §2-3): the Jacobian
...
abbrev ch6_cantor_compose := @DensePoly.cantorCompose

/-- **Cantor reduction** `DensePoly.cantorReduce ρ g D` (Trager, Chapter 6 §2-3): brings a semi-reduced
...
abbrev ch6_cantor_reduce := @DensePoly.cantorReduce

/-- **The Jacobian sum** `DensePoly.cantorAdd ρ g D₁ D₂ = cantorReduce (cantorCompose D₁ D₂)` (Trager,
Chapter 6 §2-3): the full group law `D₁ ⊕ D₂` on Mumford divisors (compose then reduce). -/
abbrev ch6_cantor_add := @DensePoly.cantorAdd

/-- **The scalar multiple** `DensePoly.cantorMul ρ g n D = n·D` (Trager, Chapter 6 §2-3): `n`-fold Jacobian
addition `D ⊕ ⋯ ⊕ D`. The iterate whose first identity hit gives the **order** (points of finite order). -/
abbrev ch6_cantor_mul := @DensePoly.cantorMul
...
is elementary. `cantorOrder` lives in `namespace …DensePoly`; the rest in `namespace …SymbolicIntegration`. -/

/-- **The divisor order** `DensePoly.cantorOrder fuel ρ g D` (Trager, Chapter 6 §2-3, "Points of Finite
...
abbrev ch6_divisor_order := @DensePoly.cantorOrder
...
`cantorReduceTracked`/`cantorMulTracked` live in `namespace …DensePoly`; `principalGenerator` and the
