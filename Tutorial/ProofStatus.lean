...
/-! Proof-status chapter: a reader-facing digest of what is formalized and proved. -/

#doc (Manual) "Proof Status" =>
...
is the verified function index `IntegrationFunctionsCatalog` and the API rendered
at `/deepwiki/api/`; this page is the map, not the territory.
...
whole differential tower by induction — completeness at the constant base $`ℚ`
...
$`ℚ`-Gaussian-elimination is proved correct, and the degree-by-degree telescoping
