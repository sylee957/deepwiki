...
- **Actual:** it first aborted while loading `DeepWiki.SymbolicIntegration.Engine.Algebraic.RadicalGeneralN` because its `.olean` file was absent; after building that module, retries similarly missed `DeepWiki.SymbolicIntegration.Engine.ResidueResultantTowerSpec` and `DeepWiki.SymbolicIntegration.RationalIntegrationLiouville`.
...
- **Workaround used:** explicitly build `DeepWiki.SymbolicIntegration.Engine.Algebraic.RadicalGeneralN`, then rerun `scripts/wiki build`.
