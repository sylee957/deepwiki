...
strong bisimulation on `Tᵤ(A)`, the generic quotient machinery
...
its symbolic state, and the region graph is finite, so untimed
bisimilarity and reachability both reduce to decidable questions on a finite
graph. All are stated
...
relation shown to be an untimed bisimulation. -/
...
/-- The configuration setoid relation is exactly the untimed bisimulation,
so it is a strong bisimulation on `Tᵤ(A)` (given the region
...
/-- The *region graph* `Tᵣ(A)` of a timed automaton: the
...
/-! ## The region graph is finite and bisimilar to the untimed system -/
...
/-- Finiteness of the region graph: for a timed automaton with finitely many
...
/-- Bisimilarity of a configuration with its symbolic state, modular form. In a
well-formed timed automaton with the region time-successor property, a configuration `(ℓ,v)` of the
...
/-- Bisimilarity of a configuration with its symbolic state, **unconditional for
finite clock sets**.
...
/-! ## Untimed bisimilarity is decidable -/

/-- Untimed bisimilarity, reduction form. Two configurations are *untimed
bisimilar* in `Tᵤ(A)` iff their symbolic states are *strongly bisimilar* in the
region graph `Tᵣ(A)`. Since the region graph is finite, strong
...
/-- Untimed bisimilarity is decidable, **unconditional for finite clock sets**. -/
...
/-- Forward direction of the reachability reduction: a reachability run of `A`
lifts to a run in the region graph (no well-formedness needed). -/
...
/-- Reachability reduction. A symbolic state `t` is reachable from
...
/-- Reachability reduction, **unconditional for finite clock sets**.
