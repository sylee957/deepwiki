...
Lifting timed bisimilarity (on TLTS states) to whole automata: two
...
/-- Timed automata `A₁` and `A₂` are *timed bisimilar* iff their initial states are
timed bisimilar in the union (disjoint sum) of the TLTSs `A₁.tlts` and `A₂.tlts`. -/
