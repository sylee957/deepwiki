...
processes are never simultaneously in their critical sections.
...
critical section is always unavailable. This reduces to a reachability statement. -/
...
/-- The mutual-exclusion safety formula `[e₁]ff ∨ [e₂]ff` — at
...
/-- The mutual-exclusion property: the invariant that the two processes are
...
/-- The mutual-exclusion property as an invariant: a state has
