...
/-! # Zones and the symbolic reachability graph
...
the *future* `Z↑` and the *reset* `Z[r]` — and the symbolic
transition relation `⤳` lift the timed-automaton semantics to
sets of valuations. The symbolic semantics is sound and
...
under the operations (the DBM theory) is cited externally. -/
...
/-- The *future* `Z↑` of a zone: all delays of its
valuations, `{v + d | v ∈ Z, d ≥ 0}`. -/
def zoneUp (Z : Set (Valuation C)) : Set (Valuation C) :=
  {w | ∃ v ∈ Z, ∃ d : ℝ≥0, w = v.add d}

/-- The *reset* `Z[r]` of a zone: reset the clocks in `r` in
...
/-- The symbolic transition relation `⤳` over symbolic
...
/-- Soundness. Whenever `(ℓ, Z) ⤳ (ℓ', Z')` and
...
/-- Completeness for action steps. A concrete action
...
/-- Completeness for delay steps. A concrete delay
