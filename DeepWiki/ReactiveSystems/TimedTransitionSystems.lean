...
actions *or* time delays `d : ℝ≥0` (Definition 9.1, `Lab = Act ∪ ℝ≥0`). Modelling
...
delay transitions — time determinism (9.2), zero delay (9.3), time additivity
(9.4) — are recorded as predicates. -/
...
delay (Definition 9.1). -/
abbrev TimedLabel (Act : Type*) := Act ⊕ ℝ≥0

/-- A timed labelled transition system (Definition 9.1): an LTS whose labels are
...
/-- Time determinism (9.2): a delay transition leads to a unique state. -/
def TimeDeterministic (T : TLTS Proc Act) : Prop :=
  ∀ ⦃p d q q'⦄, T.delay p d q → T.delay p d q' → q = q'

/-- Zero delay (9.3): every state reaches itself with a zero delay. -/
def ZeroDelay (T : TLTS Proc Act) : Prop := ∀ p, T.delay p 0 p

/-- Time additivity / continuity (9.4): a delay of `d₁ + d₂` is exactly a delay
...
/-- Timed bisimilarity is an equivalence relation (inherited from Theorem 3.1). -/
