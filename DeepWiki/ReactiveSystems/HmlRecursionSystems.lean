...
Generalises HML-with-recursion to a system of mutually recursive equations
over a variable set `V`, interpreted by simultaneous greatest/least fixed points
on the product complete lattice `V → 2^Proc`. Mixing largest and least fixed
...
/-- `O_F(ρ)` over a variable set: the states satisfying
...
/-- `O_F` is monotone in the environment (the multivariable case). -/
...
/-- `⟦D⟧`: the semantic function of a declaration `D` on the
...
/-- The largest solution of the equational system `D`: the greatest fixed
point of `⟦D⟧`. -/
def sysMax (L : LTS Proc Act) (D : V → HMLV V Act) : V → Set Proc := (sysFun L D).gfp

/-- The least solution of the equational system `D`: the least fixed point
of `⟦D⟧`. -/
...
/-- The product domain (here `V → 2^Proc`),
...
/-! ## Mixing fixed points: the livelock property -/
