...
/-! ## The fraction-field derivation extension

`fracDeriv`: a derivation on `F[X]` extends to its fraction field `K` by the quotient rule
(a self-derivation `Derivation ℤ K K`), generic in the monomial derivation on `F[X]`. -/
