...
`[CField α]`. -/
def radCase2CofactorC (fuel : ℕ) (k : ℕ) (W h C : CPolyG α) : CPolyG α :=
...
`W` passed in; `B'` is `cderivG B`, `h'` is `cderivG h`. The exact division by `W` is `cdivG`
(`W ∣ B·(½−k)W'h − C` by the cofactor congruence). With this `D`,
`radDeriv(Bf/(Wᵏy)) = C/(Wᵏy) + D/(W^{k−1}y)`. Generic over `[CField α]`. -/
def radCase2ResidualC (fuel : ℕ) (k : ℕ) (W h C B : CPolyG α) : CPolyG α :=
  let half : CPolyG α := [CField.div CField.one (cnatCastG 2)]              -- ½
  let coef := cmulG (csubG half [cnatCastG k]) (cmulG (cderivG W) h)        -- (½ − k)·W'·h
...
`B`. Checked by `cisZeroG` of `cmodG (B·(½−k)W'h − C) W`. -/
