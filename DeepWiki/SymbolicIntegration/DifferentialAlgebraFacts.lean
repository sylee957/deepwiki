...
/-! ## Faithful restatements of the Chapter 3 exercises (verification) -/

section Verification

-- **Exercise 3.1**: `D(∏ uᵢ^{eᵢ})/(∏ uᵢ^{eᵢ}) = ∑ eᵢ·Duᵢ/uᵢ`.
...
-- **Exercise 3.4**: `t₁·√−1 − 2·t₂` is a `D`-constant.
...
-- **Exercise 3.7**: `b·Δp = b·κ_D(p) + a·(dp/dt)` is a polynomial (the Leibniz form).
example {k : Type*} [Field k] [Differential k] (a b p q : k[X]) :
    bDeriv a b (p * q) = p * bDeriv a b q + q * bDeriv a b p :=
  bDeriv_mul a b p q

-- **Exercise 3.9**: squarefree `∏(t − α)` normal ⟺ `b(α)·Δα ≠ a(α)` at every root.
...
-- **Exercise 3.10**: squarefree `∏(t − α)` special ⟺ `b(α)·Δα = a(α)` at every root.
example {k : Type*} [Field k] [Differential k] (a b : k[X]) (s : Finset k) :
    IsSpecialRao a b (∏ α ∈ s, (X - C α)) ↔ ∀ α ∈ s, a.eval α = b.eval α * α′ :=
  isSpecialRao_prod_X_sub_C_iff a b s

-- **Exercise 3.11** (prime case): special prime `π` is coprime to the denominator `b`.
