...
theorem goProd_dvd (fuel : ℕ) : ∀ (fo : ℕ) (b d : DensePoly ℚ) (i : ℕ),
...
def GoExactComp (fuel : ℕ) : ℕ → DensePoly ℚ → DensePoly ℚ → Prop
  | 0, _, _ => True
  | fo + 1, b, d =>
    if b.length ≤ 1 then True
    else
...
instance decGoExactComp (fuel fo : ℕ) (b d : DensePoly ℚ) : Decidable (GoExactComp fuel fo b d) := by
...
theorem GoExactComp_to_GoExact (fuel : ℕ) : ∀ (fo : ℕ) (b d : DensePoly ℚ),
