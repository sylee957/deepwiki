...
on the dense coefficient carrier `CPoly ℚ := List ℚ`. `ratIntegrate` combines it with `lrtLogPart` for
...
/-! ### Rational-function arithmetic on `CPoly ℚ × CPoly ℚ` (numerator, denominator) -/

/-- Rational function as a `(numerator, denominator)` pair of `CPoly ℚ`s. -/
abbrev QFun := CPoly ℚ × CPoly ℚ
...
numerator `A`. -/
def hermiteInner (fuel : ℕ) (V U : CPoly ℚ) : ℕ → CPoly ℚ → QFun → QFun × CPoly ℚ
  | 0, A, g => (g, A)
  | j + 1, A, g =>
    let jval : ℚ := (j : ℚ) + 1
    let Vderiv := cderiv V
...
`∫ A/D = g + ∫ B/Dstar`. -/
def hermiteReduce (fuel : ℕ) (A D : CPoly ℚ) : QFun × QFun :=
...
def ratIntegrate (fuel : ℕ) (A D : CPoly ℚ) : QFun × List (CPoly ℚ × BPoly) :=
