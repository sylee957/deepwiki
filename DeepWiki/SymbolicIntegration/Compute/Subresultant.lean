...
def bcontentX (fuel : ℕ) (p : BPoly) : DensePoly ℚ :=
...
def credR (fuel : ℕ) (R c : DensePoly ℚ) : DensePoly ℚ := cmod fuel c R
...
def bdivC (fuel : ℕ) (p : BPoly) (c : DensePoly ℚ) : BPoly := bnorm (p.map (fun a => cdiv fuel a c))
