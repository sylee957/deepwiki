...
/-- **`A = 72x⁷+256x⁶−192x⁵−1280x⁴−312x³+1440x²+576x−96`** as a `CPoly` (Exercise 2.3 numerator),
coefficients low→high. -/
def cA23 : CPoly := [-96, 576, 1440, -312, -1280, -192, 256, 72]

/-- **`D = 9x⁸+36x⁷−32x⁶−252x⁵−78x⁴+468x³+288x²−108x+9`** as a `CPoly` (Exercise 2.3 denominator),
coefficients low→high. -/
def cD23 : CPoly := [9, -108, 288, 468, -78, -252, -32, 36, 9]
...
(`× 35` to clear the interpolation denominators): a **degree-8** integer polynomial as a `CPoly`,
...
def cR23 : CPoly := csqfreePart 80 (rtResultantCompute 80 cA23 cD23)
...
collapses the `x`-variable of a `BPoly` (`= ℚ[t][x]`) at `x = a`, leaving a `CPoly` (`= ℚ[t]`). Used to
read the LRT log argument `S(t, x)` at the integration bounds. -/
def bevalX (a : ℚ) (p : BPoly) : CPoly := p.foldr (fun c acc => cadd c (cscale a acc)) []

/-- **The log argument at the upper bound** `S₁(t, −2/3) = −2/3 + c₀(t) ∈ ℚ[t]/(R)`. -/
def cS1_23_upper : CPoly := cnorm (bevalX (-2/3) cS1_23)

/-- **The log argument at the lower bound** `S₁(t, −2) = −2 + c₀(t) ∈ ℚ[t]/(R)`. -/
def cS1_23_lower : CPoly := cnorm (bevalX (-2) cS1_23)
...
-- `logToAtanCompute_ex281`) runs on the same `CPoly` carrier the LRT output lives on. For Example
