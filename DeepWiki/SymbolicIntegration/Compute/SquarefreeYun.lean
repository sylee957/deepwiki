...
Bridges the computable `DensePoly ℚ` Yun loop to the abstract squarefree-factorization API.
-/
...
theorem csqfreeFactor_factor_assoc (fuel : ℕ) (D : DensePoly ℚ) (hex : SqfreeYun fuel D)
    (Vm : DensePoly ℚ × ℕ) (hVm : Vm ∈ csqfreeFactor fuel D) :
...
example (fuel : ℕ) (D : DensePoly ℚ) (hex : SqfreeYun fuel D)
    (Vm : DensePoly ℚ × ℕ) (hVm : Vm ∈ csqfreeFactor fuel D) :
    Squarefree (toPoly Vm.1) :=
