...
`toPoly (hermiteInner fuel V U j A g).1.2 = toPoly g.2 · (toPoly V)^m`. Each loop step `QFun.qadd`s
...
theorem hermiteInner_den_eq_pow (fuel : ℕ) (V U : DensePoly ℚ) :
  ...
  | succ j ih =>
    ...
    obtain ⟨m, hm⟩ := ih (csub (cscale (-((j : ℚ) + 1)) C) (cmul U (cderiv B))) (QFun.qadd g (B, Vpow))
    refine ⟨m + (j + 1), ?_⟩
    rw [hm]
    show toPoly (QFun.qadd g (B, Vpow)).2 * toPoly V ^ m = toPoly g.2 * toPoly V ^ (m + (j + 1))
...
`toPoly (glocIncr fuel A D Vi).2 = (toPoly Vi.1)^m`. From `hermiteInner_den_eq_pow` at the `QFun.qzero`
...
theorem glocIncr_den_eq_pow (fuel : ℕ) (A D : DensePoly ℚ) (Vi : DensePoly ℚ × ℕ) :
  ...
  rw [show (glocIncr fuel A D Vi).2
      = (hermiteInner fuel Vi.1 (DensePoly.cdivWf D
          ((List.range Vi.2).foldl (fun acc _ => cmul acc Vi.1) [1])) (Vi.2 - 1) A QFun.qzero).1.2 from rfl,
    hm]
  simp only [QFun.qzero, toPoly_eq_dense, DensePoly.toPolyG_one_singleton, one_mul]
