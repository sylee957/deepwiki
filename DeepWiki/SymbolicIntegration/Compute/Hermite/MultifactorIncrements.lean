...
def glocIncr (fuel : ℕ) (A D : CPoly ℚ) (Vi : CPoly ℚ × ℕ) : QFun :=
...
def glocList (fuel : ℕ) (A D : CPoly ℚ) (factors : List (CPoly ℚ × ℕ)) : List QFun :=
...
theorem foldl_cond_eq_foldl_glocList (fuel : ℕ) (A D : CPoly ℚ) (factors : List (CPoly ℚ × ℕ))
    (init : QFun) :
    factors.foldl
        (fun (gAcc : QFun) (Vi : CPoly ℚ × ℕ) =>
          if Vi.2 ≤ 1 then gAcc
          else
            let Vi_pow := (List.range Vi.2).foldl (fun acc _ => cmul acc Vi.1) [1]
            let U := cdiv fuel D Vi_pow
...
theorem hermiteInner_den_ne_zero (fuel : ℕ) (V U : CPoly ℚ) (hV : toPoly V ≠ 0) :
...
theorem glocIncr_den_ne_zero (fuel : ℕ) (A D : CPoly ℚ) (Vi : CPoly ℚ × ℕ) (hV : toPoly Vi.1 ≠ 0) :
