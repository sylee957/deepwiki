...
Completes `QFun = CPoly ℚ × CPoly ℚ` into a computable field ℚ(x): the field operations, the
...
theorem am_toPoly_ne_zero {p : CPoly ℚ} (hp : toPoly p ≠ 0) :
...
theorem am_cdiv_of_cmod_zero (fuel : ℕ) (p q : CPoly ℚ) (hq : cnorm q ≠ [])
...
theorem am_eq_cdiv_mul_of_cmod_zero (fuel : ℕ) (p q : CPoly ℚ) (hq : cnorm q ≠ [])
    (hrem : toPoly (cmod fuel p q) = 0) :
...
/-- `cisZero p = true ↔ toPoly p = 0`: the `CPoly ℚ` zero test agrees with vanishing in `ℚ[X]`. -/
theorem cisZero_iff_toPoly_eq_zero (p : CPoly ℚ) : cisZero p = true ↔ toPoly p = 0 := by
...
theorem toQFun_cscale_cscale (s : ℚ) (hs : s ≠ 0) (a b : CPoly ℚ) :
...
theorem toQFun_cdiv_cdiv (fuel : ℕ) (a b q : CPoly ℚ) (hq : cnorm q ≠ [])
