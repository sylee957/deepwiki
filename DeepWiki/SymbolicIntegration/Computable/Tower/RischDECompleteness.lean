...
The next-level solver over `QFunNZG β` is `crischDESolveSoundWf`; the induction hypothesis remains
...
solvable field-level RDE over `QFunNZG β`. -/
...
solvable `QFunNZG β` RDE satisfies the weak-normalizer clauses, the inner residual-tip frontier, the
...
complete at the next `QFunNZG` level. -/
