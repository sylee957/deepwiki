...
/-! # The fuel-free tower RDE instance `CRischField (QFunNZG β)`

The `CRischField (QFunNZG β)` instance that ties the tower recursion, rebased onto the **fuel-free**
`cRischDEGWf` (in place of the fuel'd `cRischDEG [1] towerRischDEFuel`). Solving an RDE at level `n+1` runs
the fuel-free §6 pipeline at level `n` and recurses into the level-`n` `crischDESolve`, bottoming at
`CRischField ℚ` — so the whole tower is now genuinely fuel-free (the old instance re-introduced fuel at every
recursion level). This file sits ABOVE `Tower.RischDEWellFounded` in the import DAG (`docs/rischde-wf-migration.md`
Phase P2). -/
...
/-- **★ `CRischField (QFunNZG β)`** (fuel-free) — the §6.1-gated, sound RDE over `β(s) = QFunNZG β`, running
the fuel-free `cRischDEGWf` over `CPolyG β = β[s]` (`Ds = [1]`) with `[CRischField β]` for the base solve,
behind the gate `cdenomNormalGateGWf`. Ties the tower recursion fuel-free, bottoming at `CRischField ℚ`. -/
...
/-! ## `native_decide` validation: the fuel-free tower RDE computes over `QFunNZG ℚ` (relocated from
`Tower/RischDEWellFounded.lean`, which is now below the instance). -/
...
open CPolyG in
/-- The generic RDE oracle `cRischDEGWf` solves `Dy = 1` over ℚ(x)(t₁) (`native_decide`, fuel-free). -/
...
open CPolyG in
/-- The generic RDE oracle solves `Dy + y = t₁ + 1` over ℚ(x)(t₁) (`native_decide`, fuel-free): the
primitive-cancellation branch with nonzero coefficient `f = 1`. -/
