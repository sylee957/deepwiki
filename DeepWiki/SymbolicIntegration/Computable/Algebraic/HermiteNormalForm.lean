...
`cdivWf`. `PolyMatrix α := List (List (CPolyG α))`; `hermiteRowReduce` returns the upper-triangular
form and `hermiteRank` its row rank over `K(x)`. Validated on `CPolyG ℚ` matrices. -/
...
A `PolyMatrix α` is a `List` of rows of `CPolyG α` entries, indexed by position; the row helpers
only swap, scale by a ring element, and subtract a ring multiple of one row from another. -/

/-- A matrix of polynomials over a computable field `α`: `List (List (CPolyG α))`, indexed by
position. -/
...
/-- Scale row `i` of a `PolyMatrix` by a polynomial `c : CPolyG α`, entrywise `cmulG`. -/
