...
/-- A basis of the kernel of a `β`-matrix `kernelBasisG nCols rows`: one vector per free (non-pivot) column
of the `gaussElimG` reduction — for free column `fc`, a `1` at `fc` with each pivot variable set to the
...
/-- Inverse of a square `n×n` matrix over a `[CField β]` `matInvG n M = some M⁻¹` (or `none` if singular):
...
`[1]`), checked by `cisZeroG (cuspNewGen − [0, 1/x])`. -/
...
in `ℚ(x)[y]/(y² − x²(x+1))`, checked by `cisZeroG (afMul f (y/x) (y/x) − (x + 1))`. -/
...
/-! ### The next pieces: the full `integralBasis` iteration
...
enlargement, and handling higher-degree (non-linear) bad primes via modular linear algebra over the
residue field `K[x]/(p)`. The Round-2 primitives are all in place; what remains is the orchestration
loop and the higher-degree residue kernel. -/
