...
theorem map_toPoly_credR {S : Type*} [CommRing S] (φ : ℚ[X] →+* S) (fuel : ℕ) (R c : CPolyQ)
...
theorem mapRingHom_toBPoly_map_credR {S : Type*} [CommRing S] (φ : ℚ[X] →+* S) (fuel : ℕ) (R : CPolyQ)
...
theorem mapRingHom_toBPoly_bredR {S : Type*} [CommRing S] (φ : ℚ[X] →+* S) (fuel : ℕ) (R : CPolyQ)
    (p : BPoly) (hR : cnorm R ≠ []) (hφR : φ (toPoly R) = 0) :
...
theorem map_toPoly_cinvMod_mul {S : Type*} [CommRing S] (φ : ℚ[X] →+* S) (fuel : ℕ) (R c : CPolyQ)
...
theorem mapRingHom_toBPoly_map_credR_cmul {S : Type*} [CommRing S] (φ : ℚ[X] →+* S) (fuel : ℕ)
    (R inv : CPolyQ) (q : BPoly) (hR : cnorm R ≠ []) (hφR : φ (toPoly R) = 0) :
...
theorem mapRingHom_toBPoly_bmonicXmodR {S : Type*} [CommRing S] (φ : ℚ[X] →+* S) (fuel : ℕ) (R : CPolyQ)
...
theorem lrtGcdCompute_isSimilar_lrtSubresultant {S : Type*} [CommRing S] [IsDomain S] (φ : ℚ[X] →+* S)
    (fuel : ℕ) (R A D : CPolyQ) (G : ℕ → BPoly) (bt : ℕ → CPolyQ) (s : ℕ → BPoly) (c : ℕ → CPolyQ) (m : ℕ)
...
example (fuel : ℕ) (p : BPoly) (c : CPolyQ) (hc : cnorm c ≠ [])
...
example (fuel : ℕ) (A D β : CPolyQ) (j : ℕ) (s : BPoly) (c : CPolyQ)
...
example (fuel : ℕ) (G : ℕ → BPoly) (bt : ℕ → CPolyQ) (s : ℕ → BPoly) (c : ℕ → CPolyQ) (j m : ℕ)
...
example {S : Type*} [CommRing S] (φ : ℚ[X] →+* S) (fuel : ℕ) (R : CPolyQ) (p : BPoly)
    (hR : cnorm R ≠ []) (hφR : φ (toPoly R) = 0) {u : ℚ} (hu : u ≠ 0)
    (hg : toPoly (cgcdExt fuel (blc (bredR fuel R p)) R).1 = Polynomial.C u)
    (hpz : ¬ bisZero (bredR fuel R p) = true) :
    (Polynomial.mapRingHom φ) (toBPoly (bmonicXmodR fuel R p))
...
def goPsi' (fuel : ℕ) (Ri_1 : BPoly) (psi : CPolyQ) (dp : ℕ) : CPolyQ :=
...
def goBeta (fuel : ℕ) (Ri_1 : BPoly) (psi : CPolyQ) (dp : ℕ) : CPolyQ :=
...
def goStep (fuel : ℕ) : BPoly × BPoly × CPolyQ × ℕ → BPoly × BPoly × CPolyQ × ℕ
  | (Ri_1, Ri, psi, dp) =>
...
def goState (fuel : ℕ) (s0 : BPoly × BPoly × CPolyQ × ℕ) : ℕ → BPoly × BPoly × CPolyQ × ℕ
  | 0 => s0
  | i + 1 => goStep fuel (goState fuel s0 i)

/-- `goState fuel (goStep fuel s₀) k = goState fuel s₀ (k+1)`. -/
theorem goState_goStep (fuel : ℕ) (s0 : BPoly × BPoly × CPolyQ × ℕ) (k : ℕ) :
...
theorem goState_succ_fst (fuel : ℕ) (s0 : BPoly × BPoly × CPolyQ × ℕ) (l : ℕ) :
...
theorem goState_fst_add_two (fuel : ℕ) (s0 : BPoly × BPoly × CPolyQ × ℕ) (l : ℕ) :
...
theorem go_step_state (fuel fo : ℕ) (s : BPoly × BPoly × CPolyQ × ℕ) (hz : ¬ bisZero s.2.1 = true) :
...
theorem go_getD (fuel : ℕ) (s : BPoly × BPoly × CPolyQ × ℕ) (k fo : ℕ) (hfo : k < fo)
...
theorem go_zero (fuel fo : ℕ) (s : BPoly × BPoly × CPolyQ × ℕ) (hz : bisZero s.2.1 = true) :
...
theorem go_eq_range (fuel : ℕ) (s : BPoly × BPoly × CPolyQ × ℕ) (k fo : ℕ) (hfo : k + 1 < fo)
    (hnz : ∀ i ≤ k, ¬ bisZero (goState fuel s i).2.1 = true)
...
theorem subresPRS_eq_range (fuel : ℕ) (P Q : BPoly) (N : ℕ) (hfo : N + 1 < fuel)
  ...
  set s0 : BPoly × BPoly × CPolyQ × ℕ := (P, Q, [-1], bdeg P - bdeg Q) with hs0
  rw [subresPRS.eq_def]
  cases N with
  | zero =>
...
noncomputable def chainBt (fuel : ℕ) (P Q : BPoly) (l : ℕ) : CPolyQ :=
