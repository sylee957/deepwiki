...
import DeepWiki.SymbolicIntegration.Compute.Hermite.InterferenceDivisibility
...
Proves the computable Hermite engine correct in `RatFunc ℚ` through the `toPoly : DensePoly ℚ → ℚ[X]`
...
example (fuel : ℕ) (A D Dstar W : DensePoly ℚ) (factors : List (DensePoly ℚ × ℕ))
...
`Dstar`. -/
theorem hermiteReduce_residual_correct_uncond' (fuel : ℕ) (A D Dstar : DensePoly ℚ)
...
example (fuel : ℕ) (A D Dstar : DensePoly ℚ) (factors : List (DensePoly ℚ × ℕ))
    (W : ℚ[X]) (hWeq : W = ((factors.filter (fun Vi => decide (2 ≤ Vi.2))).map
        (fun Vi => toPoly Vi.1 ^ (Vi.2 - 1))).prod)
    (hD : toPoly D ≠ 0) (hDstar : toPoly Dstar ≠ 0)
    (hnd : (factors.filter (fun Vi => decide (2 ≤ Vi.2))).Nodup)
