...
-- Restatements of the Gröbner-basis normal-position facts.
...
-- Geometric form: `I` is its own vanishing ideal over an algebraically closed field.
...
-- The ideal `I = ⟨A − z·D', D⟩` is zero-dimensional, with finite quotient over `K`.
example (A D : K[X]) (hM : D.Monic) (hD : D.Separable) :
    Module.Finite K
      (MvPolynomial (Fin 2) K ⧸ Ideal.span {liftX A - zVar * liftX (derivative D), liftX D}) :=
  finite_quotient_czIdeal A D hM hD

-- The ideal `I = ⟨A − z·D', D⟩` is radical, hence maximal with respect to its zero set.
example (A D : K[X]) (hD : D.Separable) :
...
-- The `z`-elimination ideal `I ∩ K[z]` is generated by the radical resultant.
open scoped Classical in
example [IsAlgClosed K] (A D : K[X]) (hD : D.Separable) (hA : A.natDegree < D.natDegree) :
    Ideal.comap polyZ (Ideal.span {liftX A - zVar * liftX (derivative D), liftX D})
      = Ideal.span {radical (rtResultant A D)} := by
  rw [← czichowskiR1_eq_radical_rtResultant A D]
  exact elimZ_eq_span_czichowskiR1 A D hD hA

-- The `z`-elimination ideal of `I` is generated by the squarefree residue resultant.
open scoped Classical in
example [IsAlgClosed K] (A D : K[X]) (hD : D.Separable) (hA : A.natDegree < D.natDegree) :
    elimZ A D = Ideal.span {czichowskiR1 A D} :=
  elimZ_eq_span_czichowskiR1 A D hD hA

-- `R₁` is the squarefree part, or radical, of the Rothstein–Trager resultant.
open scoped Classical in
example [IsAlgClosed K] (A D : K[X]) :
    czichowskiR1 A D = radical (rtResultant A D) :=
  czichowskiR1_eq_radical_rtResultant A D

-- `R₁` is monic and squarefree (the monic squarefree generator of `I ∩ K[z]`).
open scoped Classical in
example (A D : K[X]) : (czichowskiR1 A D).Monic ∧ Squarefree (czichowskiR1 A D) :=
  ⟨czichowskiR1_monic A D, czichowskiR1_squarefree A D⟩
