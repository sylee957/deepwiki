...
`IsSimilar (Sⱼ(GBPolyCore.toGBCoeffPoly (G 0), GBPolyCore.toGBCoeffPoly (G 1))) (Sⱼ(GBPolyCore.toGBCoeffPoly (G m), GBPolyCore.toGBCoeffPoly (G (m+1))))` at the elements'
own degrees. -/
theorem isSimilar_subresPRS_telescope (fuel : ℕ) (G : ℕ → GBPolyCore ℚ)
  ...
  subresultant_prs_telescope (fun i => GBPolyCore.toGBCoeffPoly (G i)) (fun l => toPoly (c l))
    (fun l => toPoly (bt l)) (fun l => GBPolyCore.toGBCoeffPoly (s l)) j m
...
structure IsSubresPRSChainInput (fuel : ℕ) (G : ℕ → GBPolyCore ℚ) (bt : ℕ → DensePoly ℚ)
  ...
  leading_coeff_ne : ∀ l ≤ m, (GBPolyCore.toGBCoeffPoly (G (l + 1))).coeff (GBPolyCore.toGBCoeffPoly (G (l + 1))).natDegree ≠ 0
  /-- Degrees strictly drop along the divided PRS chain. -/
  degree_drop : ∀ l ≤ m, (GBPolyCore.toGBCoeffPoly (G (l + 2))).natDegree < (GBPolyCore.toGBCoeffPoly (G (l + 1))).natDegree
  /-- The endpoint degree lies below all earlier second-successor degrees. -/
  endpoint_degree_lt : ∀ l < m, (GBPolyCore.toGBCoeffPoly (G (m + 2))).natDegree < (GBPolyCore.toGBCoeffPoly (G (l + 2))).natDegree
  /-- Each pseudo-quotient satisfies the degree bound used by subresultant reduction. -/
  quotient_degree_le : ∀ l ≤ m, (GBPolyCore.toGBCoeffPoly (s l)).natDegree + (GBPolyCore.toGBCoeffPoly (G (l + 1))).natDegree
    ≤ (GBPolyCore.toGBCoeffPoly (G l)).natDegree
  /-- The endpoint chain element is nonzero after `GBPolyCore.toGBCoeffPoly`. -/
  endpoint_ne_zero : GBPolyCore.toGBCoeffPoly (G (m + 2)) ≠ 0

/-- At the regular index `j = deg (GBPolyCore.toGBCoeffPoly (G (m+2)))`,
`IsSimilar (Sⱼ(GBPolyCore.toGBCoeffPoly (G 0), GBPolyCore.toGBCoeffPoly (G 1))) (GBPolyCore.toGBCoeffPoly (G (m+2)))`. -/
theorem isSimilar_subresPRS_elt (fuel : ℕ) (G : ℕ → GBPolyCore ℚ)
    ...
    IsSimilar
      (subresultant (GBPolyCore.toGBCoeffPoly (G 0)) (GBPolyCore.toGBCoeffPoly (G 1))
        (GBPolyCore.toGBCoeffPoly (G 0)).natDegree (GBPolyCore.toGBCoeffPoly (G 1)).natDegree (GBPolyCore.toGBCoeffPoly (G (m + 2))).natDegree)
      (GBPolyCore.toGBCoeffPoly (G (m + 2))) :=
  subresultant_prs_similar_elt (fun i => GBPolyCore.toGBCoeffPoly (G i)) (fun l => toPoly (c l))
    (fun l => toPoly (bt l)) (fun l => GBPolyCore.toGBCoeffPoly (s l)) m
...
`IsSimilar (lrtSubresultant A D (deg (G (m+2)))) (GBPolyCore.toGBCoeffPoly (G (m+2)))`. -/
theorem isSimilar_lrtSubresultant_subresPRS_elt (fuel : ℕ) (A D : DensePoly ℚ) (G : ℕ → GBPolyCore ℚ)
    ...
    IsSimilar (lrtSubresultant (toPoly A) (toPoly D) (GBPolyCore.toGBCoeffPoly (G (m + 2))).natDegree)
      (GBPolyCore.toGBCoeffPoly (G (m + 2))) := by
