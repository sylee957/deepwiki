...
/-- `toPoly (cC c) = C c`: the constant-`CPolyQ` lift realizes the `ℚ[X]` constant. -/
...
theorem toBPoly_liftCtoBPoly (p : CPolyQ) :
...
operand `A - t*D'`. -/
theorem toBPoly_bArgAmtD' (A D : CPolyQ) :
...
theorem lrtSubresultant_eq_subresultant_toBPoly (A D : CPolyQ) (j : ℕ) :
...
theorem lrtSubresultant_C_mul_eq_rem_of_bpsremainder (fuel : ℕ) (A D : CPolyQ) (j : ℕ)
    (s : BPoly) (c : CPolyQ)
