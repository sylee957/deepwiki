...
/-- `toPoly (cC c) = C c`: the constant-`CPoly ℚ` lift realizes the `ℚ[X]` constant. -/
...
theorem toBPoly_liftCtoBPoly (p : CPoly ℚ) :
...
operand `A - t*D'`. -/
theorem toBPoly_bArgAmtD' (A D : CPoly ℚ) :
...
theorem lrtSubresultant_eq_subresultant_toBPoly (A D : CPoly ℚ) (j : ℕ) :
...
theorem lrtSubresultant_C_mul_eq_rem_of_bpsremainder (fuel : ℕ) (A D : CPoly ℚ) (j : ℕ)
    (s : BPoly) (c : CPoly ℚ)
