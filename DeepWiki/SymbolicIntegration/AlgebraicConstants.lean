...
/-- The constants of a differential *field* form a subfield (inverses of nonzero constants are
constants: from `D(a·a⁻¹) = D 1 = 0` and `Da = 0`). -/
...
/-- **Lemma 3.3.4** (§3.3), easy inclusion `Const_D(F)(t) ⊆ Const_Δ(F(t))`: the subfield of `E`
generated by a set `S` of constants is contained in the constants. (So adjoining constants — the
images of `Const_D F` together with a constant `t` — produces only constants; the reverse
inclusion, that *every* constant of `F(t)` arises this way, is the deferred hard direction.) -/
