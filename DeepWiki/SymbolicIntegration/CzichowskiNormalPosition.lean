...
/-! ## Lemma 2.2(iii): the `z`-elimination ideal is generated by `R₁ = radical(resultant)`

Czichowski's `R₁` is the content of the first (pure-`z`) element of the reduced Gröbner basis of
`I` w.r.t. the p.l. ordering `x > z`. That first element is exactly the monic generator of the
`z`-elimination ideal `I ∩ K[z]`. We identify `I ∩ K[z]` with the principal ideal `(R₁)`, where
`R₁ = ∏_{a ∈ distinct residues}(z − a)` is the monic squarefree product over the distinct residues
`A(α)/D'(α)` — i.e. the *squarefree part / radical* of the Rothstein–Trager resultant
`R = res_x(A − z·D', D)`. The route is the variety: over `[IsAlgClosed K]`, `V(I) = {(residue, root)}`
and `I = I(V(I))` (`czIdeal_eq_vanishingIdeal_zeroLocus`), so `p(z) ∈ I ∩ K[z]` ⟺ `p` vanishes at
every residue ⟺ `R₁ ∣ p`. -/

/-- **The residue/`z` embedding** `K[z] → K[x, z]`: the univariate variable `z ↦ X 0` (matching
`zVar`). The `z`-elimination ideal `I ∩ K[z]` is the comap of `I` along this map. -/
...
/-- **Evaluating a lift at a point** reads off the `x`-coordinate: `aeval x (liftX p) = p.eval (x 1)`. -/
...
/-- **Evaluating a `z`-embedding at a point** reads off the `z`-coordinate: `aeval x (polyZ p) = p.eval (x 0)`. -/
