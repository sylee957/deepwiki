...
-- Lemma 2.2(iii): the `z`-elimination ideal `I ∩ K[z]` is `(R₁)`, `R₁ = radical(res_x(A − z·D', D))`.
open scoped Classical in
...
-- Lemma 2.2(iii): the `z`-elimination ideal of `I` is generated by Czichowski's `R₁`.
open scoped Classical in
example [IsAlgClosed K] (A D : K[X]) (hD : D.Separable) (hA : A.natDegree < D.natDegree) :
    elimZ A D = Ideal.span {czichowskiR1 A D} :=
  elimZ_eq_span_czichowskiR1 A D hD hA

-- Lemma 2.2(iii): `R₁` is the squarefree part (radical) of the Rothstein–Trager resultant.
open scoped Classical in
