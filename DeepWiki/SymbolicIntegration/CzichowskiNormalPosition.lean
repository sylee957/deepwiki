...
/-! # Czichowski's Gröbner basis `GB₁` of `⟨A − z·D', D⟩` (Bronstein §2.6 / Czichowski Lemmas 2.1, 2.2)
...
Lemma 2.2(iii) is then formalized via the variety: the `z`-elimination ideal `I ∩ K[z]`
(`elimZ`, the comap along `polyZ : z ↦ X 0`) equals the principal ideal generated by Czichowski's
`R₁ = ∏_{distinct residues}(z − a)` (`elimZ_eq_span_czichowskiR1`), the monic squarefree
**content of the first pure-`z` reduced-Gröbner-basis element for the p.l. ordering `x > z`**; and
`R₁ = radical (rtResultant A D)` is the squarefree part of the Rothstein–Trager resultant
...
/-! ### `R₁ = radical(resultant)`: the squarefree-part identification

`R₁` is the (monic) squarefree part of the Rothstein–Trager resultant `R`. Over an algebraically
closed field, every irreducible factor of a nonzero `p` is linear (`X − C a` for a root `a`), so the
**radical** `radical R` — Mathlib's product of distinct normalized prime factors — coincides with the
monic product over the distinct roots, i.e. with `R₁`. We prove `R₁ = radical R` directly:
`R₁` divides `R` and is squarefree (so `R₁ ∣ radical R`), while every prime factor of `R` divides
`R₁` (the linear-factor argument over `[IsAlgClosed K]`), giving `radical R ∣ R₁`; both monic. -/

open scoped Classical in
/-- **The radical of a polynomial over a field is monic** (it is a product of normalized irreducible,
hence monic, factors). -/
...
open scoped Classical in
/-- **`radical` divides via root containment** over `[IsAlgClosed K]`: if `q ≠ 0` and every root of
`p` is a root of `q`, then `radical p ∣ q`. Each (normalized) prime factor of `p` is a linear
`X − C a` (alg-closed), whose root `a` is a root of `p`, hence of `q`, so the factor divides `q`. -/
...
open scoped Classical in
/-- **Czichowski's `R₁` is the radical (squarefree part) of the Rothstein–Trager resultant**
(Lemma 2.2(iii), the `R₁ = radical(resultant)` identification): over an algebraically closed field,
`R₁ = ∏_{distinct roots of R}(z − a)` equals Mathlib's radical `radical (rtResultant A D)`. Both are
monic; `R₁ ∣ R` and `R₁` squarefree give `R₁ ∣ radical R`, while each prime factor of `R` is a linear
`z − a` over a root `a` of `R` (hence of `R₁`), giving the reverse divisibility — so the two monic
