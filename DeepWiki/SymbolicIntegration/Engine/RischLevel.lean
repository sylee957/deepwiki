...
/-- Formal Liouville-form integrability at one represented Risch level. -/
...
class LawfulCRischLevel (L : CRischLevel P α) : Prop where
  ...
  /-- Any formally integrable input in this level's mathematical domain produces an output. -/
  relative_complete : ∀ (Dt a d : P α),
    CPoly.toPoly d ≠ 0 → IsRischLevelIntegrable Dt a d →
...
theorem oneLevelRisch_sound (C : CMonomialCase P α) [CCanonicalRepresentation P α]
  ...
  assembleOneLevel_sound C Dt a d res hd hdegree hrun
