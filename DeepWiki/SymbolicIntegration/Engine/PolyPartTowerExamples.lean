...
def polyReduceTowerExampleDt : DensePoly (CFrac ℚ) := [qConst 1, qConst 0, qConst 1]

/-- The polynomial part `p = t³` over `ℚ(x)[t]`. -/
def polyReduceTowerExampleP : DensePoly (CFrac ℚ) := [qConst 0, qConst 0, qConst 0, qConst 1]
