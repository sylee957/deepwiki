...
theorem cIntegrateReducedG_isIntegralResult_of_lawful (Dt a d : CPoly α) (cands : List α)
    (hherm : LawfulHermiteReduction Dt a d (CPoly.cHermiteReduceTowerG Dt a d).1.1
      (CPoly.cHermiteReduceTowerG Dt a d).1.2 (CPoly.cHermiteReduceTowerG Dt a d).2.1
      (CPoly.cHermiteReduceTowerG Dt a d).2.2)
    (hres : LawfulResidueLogPart Dt (CPoly.cHermiteReduceTowerG Dt a d).2.1
      (CPoly.cHermiteReduceTowerG Dt a d).2.2 (CPoly.cIntegrateReducedG Dt a d cands).logs) :
    IsIntegralResultG Dt a d (CPoly.cIntegrateReducedG Dt a d cands) :=
...
theorem cIntegrateReducedG_primitive_isIntegralResult_via_interfaces [CharZero (CFieldSpec.K α)]
    ...
    (hcopgcd : ∀ x ∈ (CPoly.cSqfreeYunFFG d).zipIdx.filter (fun x => ¬ (x.2 + 1 ≤ 1)),
      (toPolyG (CPoly.cgcdWf (CPoly.cmulG (CPoly.cdivWf d (CPoly.cpowG x.1 (x.2 + 1)))
          (CPoly.cmonomialDeriv Dt x.1)) x.1).1).natDegree = 0
      ∧ toPolyG (CPoly.cgcdWf (CPoly.cmulG (CPoly.cdivWf d (CPoly.cpowG x.1 (x.2 + 1)))
          (CPoly.cmonomialDeriv Dt x.1)) x.1).1 ≠ 0)
    (hproper : (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.1).degree
      < (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.2).degree)
    (hDt : toPolyG Dt = Polynomial.C w)
    (hden : toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.2 = Lagrange.nodal s id)
    (hA : (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.1).degree < s.card)
    (hnorm : ∀ β ∈ s, w ≠ β′)
    (hres : CPoly.cRationalResiduesG Dt (CPoly.cHermiteReduceTowerG Dt a d).2.1
        (CPoly.cHermiteReduceTowerG Dt a d).2.2 cands = s.toList.map residueCand)
    (hDd : ∀ β ∈ s,
      (Differential.implicitDeriv (toPolyG Dt) (Lagrange.nodal s id)).eval β ≠ 0)
    (hdist : ∀ γ ∈ s, ∀ δ ∈ s, γ ≠ δ →
      (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.1).eval γ
          / (Differential.implicitDeriv (toPolyG Dt) (Lagrange.nodal s id)).eval γ
        ≠ (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.1).eval δ
          / (Differential.implicitDeriv (toPolyG Dt) (Lagrange.nodal s id)).eval δ)
    (hcand : ∀ β ∈ s, CFieldSpec.toK (residueCand β)
      = (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.1).eval β
        / (Differential.implicitDeriv (toPolyG Dt) (Lagrange.nodal s id)).eval β)
    (hgcdread : ∀ β ∈ s, Associated
      (toPolyG (CPoly.cLogArgTowerG Dt (CPoly.cHermiteReduceTowerG Dt a d).2.1
          (CPoly.cHermiteReduceTowerG Dt a d).2.2 (residueCand β)))
      (gcd (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.2)
          (toPolyG (CPoly.cAmcDdG Dt (CPoly.cHermiteReduceTowerG Dt a d).2.1
            (CPoly.cHermiteReduceTowerG Dt a d).2.2 (residueCand β))))) :
    IsIntegralResultG Dt a d (CPoly.cIntegrateReducedG Dt a d cands) :=
...
theorem cIntegrateReducedG_hyperexp_isIntegralResult_via_interfaces [CharZero (CFieldSpec.K α)]
    ...
    (hsum : ∑ β ∈ s, (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.1).eval β
        / (Differential.implicitDeriv (toPolyG Dt) (Lagrange.nodal s id)).eval β = 0)
    (hres : CPoly.cRationalResiduesG Dt (CPoly.cHermiteReduceTowerG Dt a d).2.1
        (CPoly.cHermiteReduceTowerG Dt a d).2.2 cands = s.toList.map residueCand)
    (hDd : ∀ β ∈ s,
...
theorem field_identity_of_cIntegrateReducedG_of_residueMatch_of_exact (Dt a d : CPoly α)
    ...
    (hgden : amG α (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).1.2) ≠ 0)
    (hDstar : amG α (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.2) ≠ 0)
    (hexact : amG α (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.1)
          * amG α (toPolyG (CPoly.cmulG d
              (CPoly.cmulG (CPoly.cHermiteReduceTowerG Dt a d).1.2
                (CPoly.cHermiteReduceTowerG Dt a d).1.2)))
        = amG α (toPolyG (CPoly.csubG
            (CPoly.cmulG a (CPoly.cmulG (CPoly.cHermiteReduceTowerG Dt a d).1.2
              (CPoly.cHermiteReduceTowerG Dt a d).1.2))
            (CPoly.cmulG d (CPoly.csubG
              (CPoly.cmulG (CPoly.cmonomialDeriv Dt (CPoly.cHermiteReduceTowerG Dt a d).1.1)
                (CPoly.cHermiteReduceTowerG Dt a d).1.2)
              (CPoly.cmulG (CPoly.cHermiteReduceTowerG Dt a d).1.1
                (CPoly.cmonomialDeriv Dt (CPoly.cHermiteReduceTowerG Dt a d).1.2))))))
          * amG α (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.2))
    (hmatch : ((CPoly.cIntegrateReducedG Dt a d cands).logs.map (fun cv =>
          amG α (Polynomial.C (CFieldSpec.toK cv.1))
            * (towerFractionFieldDerivG Dt (amG α (toPolyG cv.2)) / amG α (toPolyG cv.2)))).sum
        = amG α (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.1)
          / amG α (toPolyG (CPoly.cHermiteReduceTowerG Dt a d).2.2)) :
  ...
  field_identity_of_cIntegrateReducedG_of_residueMatch Dt a d cands
    (hermiteTowerStep_field_identity Dt (CPoly.cHermiteReduceTowerG Dt a d).1.1
      (CPoly.cHermiteReduceTowerG Dt a d).1.2 a d (CPoly.cHermiteReduceTowerG Dt a d).2.1
      (CPoly.cHermiteReduceTowerG Dt a d).2.2 hd hgden hDstar hexact)
    hmatch
