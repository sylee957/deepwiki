...
theorem cIntegrateCase_primitive_sound (Dt a d : DensePoly α) (cands : List α) (res : IntegralResult α)
    ...
    (hsden : toPoly ([CField.one] : DensePoly α) ≠ 0)
...
theorem cIntegrateCase_primitive_sound_polyRDE [CharZero (CFieldSpec.K α)]
    ...
    (hgden : toPoly (redNorm ([CField.one] : DensePoly α) a d cands).rational.2 ≠ 0)
    (hb : cisZero (crSpecNum ([CField.one] : DensePoly α) a d) = true)
    (hfp : cisZero (crPoly ([CField.one] : DensePoly α) a d) = false)
    (hconst : Differential.mapCoeffs (toPoly (crPoly ([CField.one] : DensePoly α) a d)) = 0)
    (hqp : cPolyRischDE ([CField.one] : DensePoly α) [] (crPoly ([CField.one] : DensePoly α) a d)
        ((cdeg (crPoly ([CField.one] : DensePoly α) a d) : ℤ) + 1) = some qp)
    (hsome : cIntegrateCase primitiveCase ([CField.one] : DensePoly α) a d cands = some res)
    (hNrmField : IsIntegralResult ([CField.one] : DensePoly α) (crNormNum ([CField.one] : DensePoly α) a d)
        (crNormDen ([CField.one] : DensePoly α) a d) (redNorm ([CField.one] : DensePoly α) a d cands))
    (hrecon : fieldFrac (crPoly ([CField.one] : DensePoly α) a d) [CField.one]
          + fieldFrac (crNormNum ([CField.one] : DensePoly α) a d) (crNormDen ([CField.one] : DensePoly α) a d)
        = fieldFrac a d) :
    IsIntegralResult ([CField.one] : DensePoly α) a d res := by
  have hone : toPoly ([CField.one] : DensePoly α) = 1 := by
    simp only [denote]
    simp
  exact cIntegrateCase_primitive_sound ([CField.one] : DensePoly α) a d cands res qp
    (fieldFrac (crPoly ([CField.one] : DensePoly α) a d) [CField.one])
    (by rw [hone]; exact one_ne_zero) hgden hb hqp hsome
    (cPolyRischDEG_nil_field_identity (crPoly ([CField.one] : DensePoly α) a d) qp _ hfp (le_refl _)
      hqp hconst)
    hNrmField hrecon
...
theorem cIntegrateCase_primitive_sound_full [CharZero (CFieldSpec.K α)]
    ...
    (hNrmField : IsIntegralResult ([CField.one] : DensePoly α) (crNormNum ([CField.one] : DensePoly α) a d)
        (crNormDen ([CField.one] : DensePoly α) a d) (redNorm ([CField.one] : DensePoly α) a d cands))
    (hd : toPoly d ≠ 0)
    (hdn : toPoly (crNormDen ([CField.one] : DensePoly α) a d) ≠ 0)
    (hds : toPoly (crSpecDen ([CField.one] : DensePoly α) a d) ≠ 0)
    (hsplit : toPoly d
      = toPoly (crSpecDen ([CField.one] : DensePoly α) a d) * toPoly (crNormDen ([CField.one] : DensePoly α) a d))
    (hgdeg : (toPoly (cgcdWf (crNormDen ([CField.one] : DensePoly α) a d)
        (crSpecDen ([CField.one] : DensePoly α) a d)).1).natDegree = 0)
    (hgne : toPoly (cgcdWf (crNormDen ([CField.one] : DensePoly α) a d)
        (crSpecDen ([CField.one] : DensePoly α) a d)).1 ≠ 0) :
    IsIntegralResult ([CField.one] : DensePoly α) a d res := by
  have hspec0 : fieldFrac (crSpecNum ([CField.one] : DensePoly α) a d)
      (crSpecDen ([CField.one] : DensePoly α) a d) = 0 := by
    simp only [fieldFrac, (cisZeroG_iff (crSpecNum ([CField.one] : DensePoly α) a d)).mp hb, map_zero,
      zero_div]
  have hrecon : fieldFrac (crPoly ([CField.one] : DensePoly α) a d) [CField.one]
        + fieldFrac (crNormNum ([CField.one] : DensePoly α) a d) (crNormDen ([CField.one] : DensePoly α) a d)
      = fieldFrac a d := by
    have h := canonicalReconstruction ([CField.one] : DensePoly α) a d hd hdn hds hsplit hgdeg hgne
