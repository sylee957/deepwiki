...
/-- Run the checked hyperexponential case through the generic one-level Risch pipeline. -/
def cIntegrateHyperexpChecked (cands : List α) (Dt a d : DensePoly α) :
    Option (IntegralResult α) :=
  cIntegrateCaseChecked hyperexpCheckedCase cands Dt a d
...
theorem DensePoly.cIntegrateHyperexpChecked_sound (cands : List α) (Dt a d : DensePoly α)
  ...
  apply DensePoly.cIntegrateCaseChecked_sound DensePoly.hyperexpCheckedCase cands Dt a d res hd hdegree
  simpa only [DensePoly.cIntegrateHyperexpChecked] using hrun
