...
`η = 1`). -/
def hyperexpDt : CPoly Lvl1 := [CField.zero, CField.one]

/-- Integrand numerator `a = 1` over `CPoly Lvl1 = ℚ(x)[t]` for `f = 1/t = 1/exp`. -/
def hyperexpInvA : CPoly Lvl1 := [CField.one]

/-- Integrand denominator `d = t = [0, 1]` over `CPoly Lvl1 = ℚ(x)[t]` for `f = 1/t = 1/exp`. -/
def hyperexpInvD : CPoly Lvl1 := [CField.zero, CField.one]
...
/-- Integrand numerator `a = t² + 1` for `f = (t²+1)/t = t + 1/t = exp + 1/exp` over `CPoly Lvl1`. -/
def hyperexpPolySpecA : CPoly Lvl1 := [CField.one, CField.zero, CField.one]
...
/-- Integrand numerator `a = 2t − 1` for `f = (2t−1)/(t²−t) = 1/t + 1/(t−1)` over `CPoly Lvl1`. -/
def hyperexpSpecNormA : CPoly Lvl1 := [CField.neg CField.one, CField.add CField.one CField.one]

/-- Integrand denominator `d = t² − t = t(t−1)` for `f = (2t−1)/(t²−t)` over `CPoly Lvl1`. -/
def hyperexpSpecNormD : CPoly Lvl1 := [CField.zero, CField.neg CField.one, CField.one]
