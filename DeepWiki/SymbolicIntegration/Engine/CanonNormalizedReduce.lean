...
def cisCanonNormalizedCore (a : QFunNZG β) : Bool :=
  cdenomNormalGate a

/-- The fuel-free wrapper canonical-normality check on the reduced denominator. -/
def cisCanonNormalized (ftilde : QFunNZG β) : Bool :=
...
example (x : QFunNZG β) : cisCanonNormalizedCore (qReduce x) = cisCanonNormalized x := rfl
...
theorem cisCanonNormalizedCoreG_qReduce_weakNormalized_iff [CFieldDomain β] (f : QFunNZG β) :
    cisCanonNormalizedCore (qReduce (weakNormalizedF f
      ...
      ↔ IsCanonNormalizedWf f
        (qOfPolyNZG (cWeakNormalizer ([CField.one] : CPoly β) f.1.1 f.1.2)) := by
