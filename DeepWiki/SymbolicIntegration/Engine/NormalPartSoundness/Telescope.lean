...
theorem amG_toPolyG_fracAddG (gAcc gloc : CPoly α × CPoly α)
    ...
    amG α (toPoly (cadd (cmul gAcc.1 gloc.2) (cmul gloc.1 gAcc.2)))
        / amG α (toPoly (cmul gAcc.2 gloc.2))
      = amG α (toPoly gAcc.1) / amG α (toPoly gAcc.2)
        + amG α (toPoly gloc.1) / amG α (toPoly gloc.2) := by
...
theorem amG_toPolyG_foldl_fracAddG :
    ∀ (glocs : List (CPoly α × CPoly α)) (s : CPoly α × CPoly α), toPoly s.2 ≠ 0 →
      ...
      toPoly res.2 ≠ 0 ∧
        amG α (toPoly res.1) / amG α (toPoly res.2)
          = amG α (toPoly s.1) / amG α (toPoly s.2)
            + (glocs.map (fun g => amG α (toPoly g.1) / amG α (toPoly g.2))).sum := by
  intro glocs
  ...
  | cons gloc rest ih =>
    ...
    rw [heq]
    have hstep : amG α (toPoly snew.1) / amG α (toPoly snew.2)
        = amG α (toPoly s.1) / amG α (toPoly s.2)
          + amG α (toPoly gloc.1) / amG α (toPoly gloc.2) := by
...
theorem towerFractionFieldDerivG_amG_fracAccG (Dt : CPoly α) (s : CPoly α × CPoly α)
    ...
    let res := glocs.foldl
      (fun (gAcc : CPoly α × CPoly α) gloc =>
        (cadd (cmul gAcc.1 gloc.2) (cmul gloc.1 gAcc.2), cmul gAcc.2 gloc.2)) s
    towerFractionFieldDerivG Dt (amG α (toPoly res.1) / amG α (toPoly res.2))
      = towerFractionFieldDerivG Dt (amG α (toPoly s.1) / amG α (toPoly s.2))
        + (glocs.map (fun g =>
            towerFractionFieldDerivG Dt (amG α (toPoly g.1) / amG α (toPoly g.2)))).sum := by
  intro res
  obtain ⟨_, heq⟩ := amG_toPolyG_foldl_fracAddG glocs s hs hmem
  show towerFractionFieldDerivG Dt (amG α (toPoly res.1) / amG α (toPoly res.2)) = _
  rw [heq, map_add]
  congr 1
...
theorem sum_towerFractionFieldDerivG_telescope (Dt : CPoly α) :
    ∀ (L₀ : CPoly α × CPoly α) (rest glocs : List (CPoly α × CPoly α)),
      ...
      (glocs.map (fun g =>
          towerFractionFieldDerivG Dt (amG α (toPoly g.1) / amG α (toPoly g.2)))).sum
        = amG α (toPoly L₀.1) / amG α (toPoly L₀.2)
          - amG α (toPoly (rest.getLastD L₀).1) / amG α (toPoly (rest.getLastD L₀).2) := by
  intro L₀ rest
...
theorem cHermiteReduceTowerG_telescope (Dt : CPoly α) (s : CPoly α × CPoly α)
    ...
    towerFractionFieldDerivG Dt
      ...
      = amG α (toPoly L₀.1) / amG α (toPoly L₀.2) := by
...
theorem towerFractionFieldDerivG_amG_seed (Dt : CPoly α) :
    towerFractionFieldDerivG Dt
        (amG α (toPoly ([CField.zero] : CPoly α)) / amG α (toPoly ([CField.one] : CPoly α))) = 0 := by
  have hzero : amG α (toPoly ([CField.zero] : CPoly α)) = 0 := by
