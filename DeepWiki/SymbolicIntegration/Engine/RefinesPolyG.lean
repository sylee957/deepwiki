...
`RefinesPolyG p q` says `p`'s denotation is `q` (`toPoly p = q`). Its use here is the
...
theorem intro (h : toPoly p = p') : RefinesPolyG p p' := h

/-- Eliminate `RefinesPolyG` to its denotation equality. -/
theorem denote_eq (h : RefinesPolyG p p') : toPoly p = p' := h

/-- `csub` respects `RefinesPolyG` — the one operation needed by the zero-test reflection below. -/
theorem sub (hp : RefinesPolyG p p') (hq : RefinesPolyG q q') :
    RefinesPolyG (csub p q) (p' - q') := by
  rw [RefinesPolyG] at hp hq ⊢
  simp only [denote]
  rw [hp, hq]

/-- A true executable zero test reflects to zero of any refined semantic polynomial. -/
theorem eq_zero_of_cisZero (hp : RefinesPolyG p p') (hz : cisZero p = true) : p' = 0 := by
  rw [← hp]; exact (cisZeroG_iff p).mp hz

/-- A false executable zero test reflects to nonzero of any refined semantic polynomial. -/
theorem ne_zero_of_cisZero_false (hp : RefinesPolyG p p') (hz : cisZero p = false) : p' ≠ 0 := by
  intro hzero
