...
def sqrtComponentCurve : CPoly (QFunNZG ℚ) := [qxOfNum [0, -1], CField.zero, CField.one]

/-- The square-root component integral `F₁ = (2/3)·x·y`. -/
def sqrtComponentIntegral : CPoly (QFunNZG ℚ) := [CField.zero, qxOfNum [0, 2/3]]

/-- The cube-root component curve `T₂ = y³ − x ∈ ℚ(x)[y]`. -/
def cubeRootComponentCurve : CPoly (QFunNZG ℚ) := [qxOfNum [0, -1], CField.zero, CField.zero, CField.one]

/-- The cube-root component integral `F₂ = (3/4)·x·y`. -/
def cubeRootComponentIntegral : CPoly (QFunNZG ℚ) := [CField.zero, qxOfNum [0, 3/4]]
...
-- `CPoly.afIntegrateFunctionAlgebra_sound`. The recombined integral `F = Σ eᵢ Fᵢ` of the function
