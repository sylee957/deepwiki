...
noncomputable def toPoly2S (p : List (DensePoly ℚ)) : SGauss[X] := (toPoly2 p).map (AdjoinRoot.of _)
...
theorem toPoly2S_cons (a : DensePoly ℚ) (as : List (DensePoly ℚ)) :
...
theorem evalAtI_spec (p : List (DensePoly ℚ)) :
  ...
  | cons a as ih =>
    rw [toPoly2S_cons, eval_add, eval_C, eval_mul, eval_X, ih]
...
theorem toPoly2_cscaleListQ (s : ℚ) (p : List (DensePoly ℚ)) :
  ...
  · rw [List.getD_eq_getElem?_getD, List.getElem?_map,
    ...
    simp
theorem toPoly2S_cscaleListQ (s : ℚ) (p : List (DensePoly ℚ)) :
...
theorem toPoly2S_tanDeriv (p : List (DensePoly ℚ)) :
    toPoly2S (tanDeriv p)
...
theorem toPoly2S_coeff (p : List (DensePoly ℚ)) (k : ℕ) :
...
theorem pairListToS_zip (a b : List (DensePoly ℚ)) (N : ℕ) (ha : a.length ≤ N) (hb : b.length ≤ N) :
  ...
  apply Polynomial.ext; intro k
  rw [pairListToS_coeff, Polynomial.coeff_add, Polynomial.coeff_C_mul, toPoly2S_coeff,
    toPoly2S_coeff]
  by_cases hk : k < N
...
theorem cplx_eq_imp (p q r s : List (DensePoly ℚ)) (h : cplx p q = cplx r s) :
  ...
  have hz : Polynomial.map (AdjoinRoot.of _) (toPoly2 p - toPoly2 r)
      + C iU * Polynomial.map (AdjoinRoot.of _) (toPoly2 q - toPoly2 s) = 0 := by
    unfold cplx toPoly2S at h
    rw [Polynomial.map_sub, Polynomial.map_sub]
    linear_combination h
  -- coefficient-wise apply of_add_iU_eq_zero, then map_of injective
  have hcoeff : ∀ k, (toPoly2 p - toPoly2 r).coeff k = 0 ∧ (toPoly2 q - toPoly2 s).coeff k = 0 := by
    intro k
...
theorem mapDeriv_tsub (a b : List (DensePoly ℚ)) :
    toPoly2 ((tsub a b).map cderivQ)
      = toPoly2 (a.map cderivQ) - toPoly2 (b.map cderivQ) := by
  apply Polynomial.ext; intro k
  rw [mapDeriv_coeff, toPoly2_tsub, Polynomial.coeff_sub, derivative_sub,
    Polynomial.coeff_sub, mapDeriv_coeff, mapDeriv_coeff]

/-- `tanDeriv` additive over `tadd` (at the `toPoly2` level). -/
theorem toPoly2_tanDeriv_tadd (a b : List (DensePoly ℚ)) :
...
theorem toPoly2_tanDeriv_tsub (a b : List (DensePoly ℚ)) :
...
theorem toPoly2_shift (h : List (DensePoly ℚ)) : toPoly2 ([] :: h) = X * toPoly2 h := by
  rw [toPoly2_cons, toPolyG_nil, map_zero, zero_add]
theorem toPoly2_singleton (s : DensePoly ℚ) : toPoly2 [s] = C (toPoly s) := by
  rw [show ([s] : List (DensePoly ℚ)) = s :: [] from rfl, toPoly2_cons, toPoly2_nil, mul_zero, add_zero]
...
theorem toPoly2_tanDeriv_shift (h : List (DensePoly ℚ)) :
...
theorem toPoly2_tanDeriv_singleton (s : DensePoly ℚ) :
    toPoly2 (tanDeriv [s]) = C (toPoly (cderivQ s)) := by
...
theorem eval_toPoly2S (p : List (DensePoly ℚ)) :
    (toPoly2S p).eval iU = pairToS (evalAtI p) := evalAtI_spec p

theorem eval_toPoly2S_singleton (s : DensePoly ℚ) : (toPoly2S [s]).eval iU = toS s := by
  rw [show toPoly2S [s] = C (toS s) from by
    rw [toPoly2S_eq_map, toPoly2_singleton, map_C]; rfl, eval_C]
theorem eval_toPoly2S_shift (h : List (DensePoly ℚ)) :
...
theorem numerator_eval_zero (c1 c2 : List (DensePoly ℚ)) (s1 s2 : DensePoly ℚ) (nN : ℚ) :
  ...
  rw [toPoly2S_tadd, toPoly2S_tadd, toPoly2S_tsub, toPoly2S_tsub, toPoly2S_cscaleListQ,
    toPoly2S_cscaleListQ, toPoly2S_tadd, toPoly2S_tsub]
  simp only [eval_add, eval_sub, eval_mul, eval_C]
  rw [eval_toPoly2S c1, eval_toPoly2S c2, eval_toPoly2S_singleton, eval_toPoly2S_singleton,
    eval_toPoly2S_singleton, eval_toPoly2S_singleton, eval_toPoly2S_shift, eval_toPoly2S_shift,
    eval_toPoly2S_singleton, eval_toPoly2S_singleton]
...
theorem toPoly2S_shift (h : List (DensePoly ℚ)) : toPoly2S ([] :: h) = X * toPoly2S h := by
...
/-- `tdeg p = 0 ⟹ toPoly2 p = C (toPoly (tcoeff p 0))` (only the `t⁰`-coefficient survives). -/
theorem tdeg_zero_toPoly2 (p : List (DensePoly ℚ)) (h : tdeg p = 0) :
    toPoly2 p = C (toPoly (tcoeff p 0)) := by
  apply Polynomial.ext; intro k
  rw [toPoly2_coeff]
  cases k with
  | zero => rw [tcoeff]; simp
...
def TanSolves (b0 b2 : DensePoly ℚ) (n : ℕ) (c1 c2 q1 q2 : List (DensePoly ℚ)) : Prop :=
  toPoly2 (tanDeriv q1)
      + (C (toPoly b0) - C (C ((n : ℚ))) * X) * toPoly2 q1
      - C (toPoly b2) * toPoly2 q2 = toPoly2 c1
    ∧ toPoly2 (tanDeriv q2)
      + C (toPoly b2) * toPoly2 q1
  ...
  | succ m ih =>
    ...
    · rw [hbase] at hsome
      ...
      · rw [hrec] at hsome
        ...
        obtain ⟨hI1, hI2⟩ := hih
        -- reduction: toPoly2 realNum = X*D1 + D2 ; toPoly2 imagNum = X*D2 - D1.
        rw [show toPoly2 (tadd ([] :: quot.map Prod.fst) (quot.map Prod.snd)) = X * D1 + D2 from by
              rw [toPoly2_tadd, toPoly2_shift],
            show toPoly2 (tsub ([] :: quot.map Prod.snd) (quot.map Prod.fst)) = X * D2 - D1 from by
              rw [toPoly2_tsub, toPoly2_shift]] at hred
        obtain ⟨hR1, hR2⟩ := hred
        -- expand toPoly2 of realNum / imagNum (as ℚ[x][t]).
        have hRexp : toPoly2 realNum
            = (toPoly2 c1 - C (toPoly z1)) + C (C nN) * (X * C (toPoly s1) + C (toPoly s2)) := by
          rw [hrN, toPoly2_tadd, toPoly2_tsub, toPoly2_singleton, toPoly2_cscaleListQ, toPoly2_tadd,
            toPoly2_shift, toPoly2_singleton, toPoly2_singleton]
        have hIexp : toPoly2 imagNum
            = (toPoly2 c2 - C (toPoly z2)) + C (C nN) * (X * C (toPoly s2) - C (toPoly s1)) := by
          rw [hiN, toPoly2_tadd, toPoly2_tsub, toPoly2_singleton, toPoly2_cscaleListQ, toPoly2_tsub,
            toPoly2_shift, toPoly2_singleton, toPoly2_singleton]
        ...
        have hQ1 : toPoly2 (tadd (tadd ([[]] ++ h1) h2) [s1]) = X * H1 + H2 + C (toPoly s1) := by
          rw [hsh1, toPoly2_tadd, toPoly2_tadd, toPoly2_shift, toPoly2_singleton]
        have hQ2 : toPoly2 (tsub (tadd ([[]] ++ h2) [s2]) h1) = X * H2 + C (toPoly s2) - H1 := by
          rw [hsh2, toPoly2_tsub, toPoly2_tadd, toPoly2_shift, toPoly2_singleton]
        have hDQ1 : toPoly2 (tanDeriv (tadd (tadd ([[]] ++ h1) h2) [s1]))
            = (X * DH1 + (X ^ 2 + 1) * H1) + DH2 + C (derivative (toPoly s1)) := by
          rw [hsh1, toPoly2_tanDeriv_tadd, toPoly2_tanDeriv_tadd, toPoly2_tanDeriv_shift,
            toPoly2_tanDeriv_singleton, cderivQ, toPolyG_cderivG]
        have hDQ2 : toPoly2 (tanDeriv (tsub (tadd ([[]] ++ h2) [s2]) h1))
            = (X * DH2 + (X ^ 2 + 1) * H2) + C (derivative (toPoly s2)) - DH1 := by
          rw [hsh2, toPoly2_tanDeriv_tsub, toPoly2_tanDeriv_tadd, toPoly2_tanDeriv_shift,
            toPoly2_tanDeriv_singleton, cderivQ, toPolyG_cderivG]
...
theorem tisZero_of_toPoly2_zero (p : List (DensePoly ℚ)) (h : toPoly2 p = 0) : tisZero p = true := by
  rw [tisZero, List.all_eq_true]
  intro a ha
  obtain ⟨k, hk, rfl⟩ := List.getElem_of_mem ha
  rw [cisZeroG_iff]
  have hc := toPoly2_coeff p k
...
theorem cancelTanClearedCheck_of_reconstruct (dbound : ℕ) (b0 b2 : DensePoly ℚ)
  ...
  · apply tisZero_of_toPoly2_zero
    ...
    rw [toPoly2_tsub, toPoly2_tadd, toPoly2_tanDeriv, toPoly2_tadd, toPoly2_tsub,
      toPoly2_map_cmulG, toPoly2_map_cmulG, toPoly2_mulT, toPoly2_twoT]
  ...
  · apply tisZero_of_toPoly2_zero
    rw [toPoly2_tsub, toPoly2_tadd, toPoly2_tanDeriv, toPoly2_tadd, toPoly2_map_cmulG, toPoly2_tsub,
      toPoly2_map_cmulG, toPoly2_mulT, toPoly2_twoT]
    rw [toPoly2_tanDeriv] at hG2
...
theorem cCoupledDECancelTan_sound (dbound : ℕ) (b0 b2 : DensePoly ℚ)
    ...
    (toPoly2 (q1.map cderivQ) + (Polynomial.X ^ 2 + 1) * Polynomial.derivative (toPoly2 q1))
        + (Polynomial.C (toPoly b0) * toPoly2 q1
            - Polynomial.C (Polynomial.C 2) * Polynomial.X * toPoly2 q1)
        + Polynomial.C (toPoly (cscale (-1) b2)) * toPoly2 q2
      = toPoly2 c1 ∧
      (toPoly2 (q2.map cderivQ) + (Polynomial.X ^ 2 + 1) * Polynomial.derivative (toPoly2 q2))
        + Polynomial.C (toPoly b2) * toPoly2 q1
        + (Polynomial.C (toPoly b0) * toPoly2 q2
