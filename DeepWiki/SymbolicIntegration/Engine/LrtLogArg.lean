...
def cLrtLogArgG (Dt hNum Dstar : CPoly α) : List (CPoly α × List (CPoly α)) :=
  ...
  (cSqfreeYunFFG R).zipIdx.filterMap (fun (Ri, idx) =>
    ...
    else if i = n then some (Ri, Dstar.map (fun c => ([c] : CPoly α)))
...
theorem cResidueResultant_invT2m1 :
    cResidueResultantTowerG ([1] : CPoly ℚ) [1] [-1, 0, 1] = [1, 0, -4] := by native_decide
...
theorem cLrtLogArg_invT2m1 :
    cLrtLogArgG ([1] : CPoly ℚ) [1] [-1, 0, 1] = [([-1/4, 0, 1], [[1], [0, -2]])] := by native_decide
...
theorem cLrtLogArg_invT_pureLog :
    (cLrtLogArgG ([1] : CPoly ℚ) [1] [0, 1]).map (·.2) = [[[0], [1]]] := by native_decide
...
theorem cLrtLogArg_invT2p1_pureLog :
    (cLrtLogArgG ([1] : CPoly ℚ) [0, 2] [1, 0, 1]).map (·.2) = [[[1], [0], [1]]] := by native_decide
