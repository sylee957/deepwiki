...
def cLrtLogArg (Dt hNum Dstar : CPoly α) : List (CPoly α × List (CPoly α)) :=
...
theorem cResidueResultant_invT2m1 :
    cResidueResultantTower ([1] : CPoly ℚ) [1] [-1, 0, 1] = [1, 0, -4] := by native_decide
...
theorem cLrtLogArg_invT2m1 :
    cLrtLogArg ([1] : CPoly ℚ) [1] [-1, 0, 1] = [([-1/4, 0, 1], [[1], [0, -2]])] := by native_decide
...
theorem cLrtLogArg_invT_pureLog :
    (cLrtLogArg ([1] : CPoly ℚ) [1] [0, 1]).map (·.2) = [[[0], [1]]] := by native_decide
...
theorem cLrtLogArg_invT2p1_pureLog :
    (cLrtLogArg ([1] : CPoly ℚ) [0, 2] [1, 0, 1]).map (·.2) = [[[1], [0], [1]]] := by native_decide
