...
theorem towerCoeffIntegrateLrt_sound (c b : QFunNZG β) (h : towerCoeffIntegrateLrt c = some b) :
  ...
  have hcd : CFieldSpec.toK (CDiffField.cderiv (CField.div (qEmbedNumG bn) (qEmbedNumG bd)))
    ...
    rfl
  have htoK_embed : ∀ num : CPoly β, CFieldSpec.toK (qEmbedNumG num) = QFunNZG.amG β (toPoly num) := by
    intro num
    show QFunNZG.amG β (toPoly num) / QFunNZG.amG β (toPoly ([CField.one] : CPoly β))
      = QFunNZG.amG β (toPoly num)
    simp only [denote, map_one, mul_zero, add_zero, div_one]
  have htoK_c : CFieldSpec.toK c
      = QFunNZG.amG β (toPoly (qnumCoeffG c)) / QFunNZG.amG β (toPoly (qdenCoeffG c)) := rfl
...
theorem tower_special_identityLrt (Dt fp qp : CPoly (QFunNZG β)) (hDt : toPoly Dt = 1)
  ...
  have hone : toPoly ([CField.one] : CPoly (QFunNZG β)) = 1 := by
    simp only [denote, map_one, mul_zero, add_zero]
  have hbridge : towerFractionFieldDerivG Dt (QFunNZG.amG (QFunNZG β) (toPoly qp))
      = QFunNZG.amG (QFunNZG β) (Differential.implicitDeriv (toPoly Dt) (toPoly qp)) := by
