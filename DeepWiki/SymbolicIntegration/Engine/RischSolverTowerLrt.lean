...
for `(QFunNZ β)(t)`. The polynomial part's coefficient integration recurses into `LawfulRischLevelLrt β`'s
...
/-- Embed a polynomial as a fraction `num/1 ∈ QFunNZ β`. -/
def qEmbedNum {β : Type*} [CField β] [CFieldDomain β] (num : CPoly β) : QFunNZ β :=
  ⟨(num, [CField.one]), QFunNZ.cisZeroG_one_singleton⟩

/-- Integrate a coefficient `c ∈ QFunNZ β = β(s)` by recursing into
`LawfulRischLevelLrt β.integrateRationalLrt` (the log-free LRT integrator, whose soundness is descent-free
`K`-level via the ∀E ⇒ K bridge) with the carrier derivation `Ds = [1]`, reassembling via `QFunNZ β` field
...
def towerCoeffIntegrateLrt (c : QFunNZ β) : Option (QFunNZ β) :=
...
`QFunNZ β` field division that `towerCoeffIntegrateLrt` performs. -/
theorem towerCoeffIntegrateLrt_sound (c b : QFunNZ β) (h : towerCoeffIntegrateLrt c = some b) :
  ...
  have hcd : CFieldSpec.toK (CDiffField.cderiv (CField.div (qEmbedNum bn) (qEmbedNum bd)))
    ...
    rfl
  have htoK_embed : ∀ num : CPoly β, CFieldSpec.toK (qEmbedNum num) = QFunNZ.am β (toPoly num) := by
    intro num
    show QFunNZ.am β (toPoly num) / QFunNZ.am β (toPoly ([CField.one] : CPoly β))
      = QFunNZ.am β (toPoly num)
    simp only [denote, map_one, mul_zero, add_zero, div_one]
  have htoK_c : CFieldSpec.toK c
      = QFunNZ.am β (toPoly (qnumCoeff c)) / QFunNZ.am β (toPoly (qdenCoeff c)) := rfl
...
`QFunNZ β` elements), falling back to the log-free `towerCoeffIntegrateLrt` (`c = 0`) when the class supplies no
...
def towerCoeffIntegrateSingleLrt (η c : QFunNZ β) : Option (QFunNZ β × QFunNZ β) :=
...
def towerPolyIntegrateLrt (η : QFunNZ β) (p : CPoly (QFunNZ β)) : Option (CPoly (QFunNZ β)) :=
...
theorem towerPolyIntegrateLrt_sound (η : QFunNZ β) (p q : CPoly (QFunNZ β))
...
theorem tower_special_identityLrt (Dt fp qp : CPoly (QFunNZ β)) (hDt : toPoly Dt = 1)
  ...
  have hone : toPoly ([CField.one] : CPoly (QFunNZ β)) = 1 := by
    simp only [denote, map_one, mul_zero, add_zero]
  have hbridge : towerFractionFieldDeriv Dt (QFunNZ.am (QFunNZ β) (toPoly qp))
      = QFunNZ.am (QFunNZ β) (Differential.implicitDeriv (toPoly Dt) (toPoly qp)) := by
...
def towerPrimitiveCaseLrt : MonomialCase (QFunNZ β) where
  ...
  reducedCorrect := (primitiveGuardedCase (α := QFunNZ β)).reducedCorrect
...
tower instance. -/
theorem towerPrimitiveCaseLrt_specialSound [Fact (GcdFFCorrect (α := QFunNZ β))]
    (Dt a d snum sden : CPoly (QFunNZ β)) (hd0 : toPoly d ≠ 0)
    (hhook : (towerPrimitiveCaseLrt (β := β)).integrateSpecial Dt (crPoly Dt a d) (crSpecNum Dt a d)
      (crSpecDen Dt a d) = some (snum, sden)) :
    toPoly sden ≠ 0 ∧ ∃ v : RatFunc (CFieldSpec.K (QFunNZ β)),
...
/-- The LRT tower step instance: `LawfulRischLevelLrt (QFunNZ β)` from a below-level LRT solver
`[LawfulRischLevelLrt β]` and this level's reduced LRT frontier `[PrimitiveFrontierLrt (QFunNZ β)]`.
...
instance instLawfulRischLevelLrtTower [Fact (GcdFFCorrect (α := QFunNZ β))]
    [PrimitiveFrontierLrt (QFunNZ β)] : LawfulRischLevelLrt (QFunNZ β) where
...
noncomputable example [Fact (GcdFFCorrect (α := QFunNZ β))] [PrimitiveFrontierLrt (QFunNZ β)]
    [Fact (GcdFFCorrect (α := QFunNZ (QFunNZ β)))] [PrimitiveFrontierLrt (QFunNZ (QFunNZ β))] :
    LawfulRischLevelLrt (QFunNZ (QFunNZ β)) := inferInstance
