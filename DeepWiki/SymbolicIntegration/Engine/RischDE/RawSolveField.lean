...
theorem cdegG_one_eq_zero_wf : cdeg ([CField.one] : DensePoly β) = 0 := by
  have hnz : DensePoly.cisZero ([CField.one] : DensePoly β) = false := CFieldDomain.nz_one
  rw [DensePoly.cisZero, List.isEmpty_eq_false_iff_exists_mem] at hnz
  have hcn : DensePoly.cnorm ([CField.one] : DensePoly β) = [CField.one] := by
    show (if CField.isZero (CField.one : β) then ([] : DensePoly β) else [CField.one]) = [CField.one]
    by_cases h1 : CField.isZero (CField.one : β) = true
    · exfalso
      obtain ⟨a, ha⟩ := hnz
      rw [show DensePoly.cnorm ([CField.one] : DensePoly β)
          = (if CField.isZero (CField.one : β) then ([] : DensePoly β) else [CField.one]) from rfl,
        if_pos h1] at ha
...
structure RawSolveResidualWf (ftilde gtilde : CFrac β) : Prop where
  ...
  hres : ∀ a0 b0 c0 h0 : DensePoly β,
    cRdeNormalDenominator ([CField.one] : DensePoly β) ftilde.1.1 ftilde.1.2 gtilde.1.1 gtilde.1.2
        = some (a0, b0, c0, h0) →
      RischDEStructuralResidualWf ([CField.one] : DensePoly β) ftilde.1.1 ftilde.1.2 gtilde.1.1 gtilde.1.2
        a0 b0 c0 h0
  ...
  hdb : ∀ a0 b0 c0 bbar cbar : DensePoly β, ∀ m : ℤ, ∀ α' β' : DensePoly β,
    cSPDE ([CField.one] : DensePoly β)
        (cRdeSpecialDenominator ([CField.one] : DensePoly β) a0 b0 c0).1
        (cRdeSpecialDenominator ([CField.one] : DensePoly β) a0 b0 c0).2.1
        (cRdeSpecialDenominator ([CField.one] : DensePoly β) a0 b0 c0).2.2.1
        (cRdeBoundDegree ([CField.one] : DensePoly β)
          (cRdeSpecialDenominator ([CField.one] : DensePoly β) a0 b0 c0).1
          (cRdeSpecialDenominator ([CField.one] : DensePoly β) a0 b0 c0).2.1
...
theorem crischDERawSolveWf_field_of_residual (ftilde gtilde y : CFrac β)
  ...
  rcases hsucc : DensePoly.cRischDE ([CField.one] : DensePoly β) ftilde.1.1 ftilde.1.2 gtilde.1.1 gtilde.1.2
  ...
  · simp only at hsolve
    ...
    · rw [dif_pos hyz, Option.some.injEq] at hsolve
      ...
      rw [hy1, hy2]
      exact crischDEWf_field_of_success_and_residual ([CField.one] : DensePoly β)
