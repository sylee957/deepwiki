...
theorem cdegG_one_eq_zero_wf : cdegG ([CField.one] : CPoly β) = 0 := by
  have hnz : CPoly.cisZeroG ([CField.one] : CPoly β) = false := CFieldDomain.nz_one
  rw [CPoly.cisZeroG, List.isEmpty_eq_false_iff_exists_mem] at hnz
  have hcn : CPoly.cnormG ([CField.one] : CPoly β) = [CField.one] := by
    show (if CField.isZero (CField.one : β) then ([] : CPoly β) else [CField.one]) = [CField.one]
    by_cases h1 : CField.isZero (CField.one : β) = true
    · exfalso
      obtain ⟨a, ha⟩ := hnz
      rw [show CPoly.cnormG ([CField.one] : CPoly β)
...
structure RawSolveResidualWf (ftilde gtilde : QFunNZG β) : Prop where
  ...
  hres : ∀ a0 b0 c0 h0 : CPoly β,
    cRdeNormalDenominatorG ([CField.one] : CPoly β) ftilde.1.1 ftilde.1.2 gtilde.1.1 gtilde.1.2
        = some (a0, b0, c0, h0) →
  ...
  hdb : ∀ a0 b0 c0 bbar cbar : CPoly β, ∀ m : ℤ, ∀ α' β' : CPoly β,
    cSPDEG ([CField.one] : CPoly β)
        (cRdeSpecialDenominatorG ([CField.one] : CPoly β) a0 b0 c0).1
        (cRdeSpecialDenominatorG ([CField.one] : CPoly β) a0 b0 c0).2.1
        (cRdeSpecialDenominatorG ([CField.one] : CPoly β) a0 b0 c0).2.2.1
        (cRdeBoundDegreeG ([CField.one] : CPoly β)
          (cRdeSpecialDenominatorG ([CField.one] : CPoly β) a0 b0 c0).1
          (cRdeSpecialDenominatorG ([CField.one] : CPoly β) a0 b0 c0).2.1
...
theorem crischDERawSolveWf_field_of_residual (ftilde gtilde y : QFunNZG β)
  ...
  rw [show crischDERawSolveWf ftilde gtilde
      = (match CPoly.cRischDEG ([CField.one] : CPoly β) ftilde.1.1 ftilde.1.2 gtilde.1.1 gtilde.1.2 with
         | none => none
         | some (ynum, yden) =>
           if h : CPoly.cisZeroG yden = false then some ⟨(ynum, yden), h⟩ else none) from rfl] at hsolve
  rcases hsucc : CPoly.cRischDEG ([CField.one] : CPoly β) ftilde.1.1 ftilde.1.2 gtilde.1.1 gtilde.1.2
    with _ | ⟨ynum, yden⟩ <;> rw [hsucc] at hsolve
  · exact absurd hsolve (by simp)
  · simp only at hsolve
    by_cases hyz : CPoly.cisZeroG yden = false
