...
@[denote] theorem toPolyG_cone_eq_one_wf : toPolyG ([CField.one] : CPoly β) = 1 := by
  simp only [denote]
  simp
...
theorem cgcdFFCoreWf_one_isUnit (z : CPoly β) :
    IsUnit (toPolyG (CFracGcdCoreWf.cgcdFFCoreWf ([CField.one] : CPoly β) z)) := by
...
theorem cdegG_cdivWf_zero_of_unit_divisor_wf (c d : CPoly β)
    (hc : cdegG c = 0) (hd0 : CPoly.cnormG d ≠ []) (hd : cdegG d = 0) :
    cdegG (CPoly.cdivWf c d) = 0 := by
  have hdlen : (CPoly.cnormG d : List β).length = 1 := by
    rw [cdegG] at hd
    have : 0 < (CPoly.cnormG d : List β).length := List.length_pos_iff.mpr hd0
    omega
  have hrem := CPoly.cmodWf_length_lt c d hd0
  rw [hdlen] at hrem
  have hremnil : CPoly.cnormG (CPoly.cmodWf c d) = [] := List.length_eq_zero_iff.mp (by omega)
  have hrem0 : toPolyG (CPoly.cdivmodWf c d).2 = 0 := by
  ...
  have hdne : toPolyG d ≠ 0 := fun h => hd0 ((CPoly.cnormG_eq_nil_iff d).mpr h)
  ...
  by_cases hquo0 : toPolyG (CPoly.cdivmodWf c d).1 = 0
  · rw [hquo0]; simp
...
theorem cdegG_cstepG_one : cdegG (CPoly.cstepG ([CField.one] : CPoly β) [CField.one]) = 0 := by
  rw [CPoly.cstepG]
  set g1 := CFracGcdCoreWf.cgcdFFCoreWf ([CField.one] : CPoly β)
    (CPoly.cmonomialDeriv [CField.one] [CField.one]) with hg1
  set g2 := CFracGcdCoreWf.cgcdFFCoreWf ([CField.one] : CPoly β) (CPoly.cderivG [CField.one]) with hg2
  ...
  have hg20 : CPoly.cnormG g2 ≠ [] := by
    intro he; have hz : toPolyG g2 = 0 := (CPoly.cnormG_eq_nil_iff g2).mp he
...
theorem cSplitFactorFastG_one_eq :
    CPoly.cSplitFactorFastG ([CField.one] : CPoly β) [CField.one]
      = ([CField.one], [CField.one]) := by
  rw [CPoly.cSplitFactorFastG, if_pos cdegG_cstepG_one]

/-- `cdegG (cSpecialPolyG [1]) = 0`: the special part of the primitive monomial `[1]` is constant. -/
theorem cdegG_cSpecialPolyG_one_eq_zero :
    cdegG (CPoly.cSpecialPolyG ([CField.one] : CPoly β)) = 0 := by
  rw [CPoly.cSpecialPolyG, cSplitFactorFastG_one_eq, cdegG_eq_natDegree]
...
example {β : Type*} [CField β] [CFieldSpec β] [CDiffField β] [CFracGcdCoreWf β] [CTowerGcdWitnessWf β] :
    cdegG (CPoly.cSpecialPolyG ([CField.one] : CPoly β)) = 0 :=
