...
/-! # Tower-level demos for the `QFunNZ` gcd-cancel reducer `qReduce`
...
/-- The fraction `t/(t−1) ∈ QFunNZ ℚ` (numerator `[0,1]`, denominator `[−1,1]`). -/
def swellA : QFunNZ ℚ := ⟨([(0 : ℚ), 1], [(-1 : ℚ), 1]), by native_decide⟩

/-- The fraction `(t−1)/t ∈ QFunNZ ℚ` (numerator `[−1,1]`, denominator `[0,1]`), the reciprocal of
`swellA`. -/
def swellB : QFunNZ ℚ := ⟨([(-1 : ℚ), 1], [(0 : ℚ), 1]), by native_decide⟩

/-- The unreduced product `swellA · swellB = (t·(t−1))/((t−1)·t)` via `qmulNZ`: both num and den are
`t²−t` (length 3) though the value is `1`. -/
def swellProd : QFunNZ ℚ := qmulNZ swellA swellB
...
/-- `qReduce` preserves the field value: `toQFunNZ (qReduce swellProd) = toQFunNZ swellProd`. -/
theorem swellProd_value_preserved :
    toQFunNZ (qReduce swellProd) = toQFunNZ swellProd :=
...
`QFunNZ.isZeroNZG_qReduce`): a reduced fraction tests zero exactly when the original does. -/
...
theorem Rstuck_eq_one : CField.isZero (CField.sub Rstuck (CField.one : QFunNZ ℚ)) = true := by
  native_decide
...
theorem Rstuck_unreduced_returns_none :
    CRischField.crischDESolve (CField.zero : QFunNZ ℚ) Rstuck = none := by native_decide

/-- The reduced residual solves: `crischDESolve 0 (qReduce Rstuck)` returns `some y` with `y = x`,
recovering `∫1 = x`. -/
theorem Rstuck_reduced_solves :
    (match CRischField.crischDESolve (CField.zero : QFunNZ ℚ) (qReduce Rstuck) with
