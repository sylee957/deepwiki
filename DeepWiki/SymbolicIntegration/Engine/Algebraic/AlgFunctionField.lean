...
def afBasisElem (i : ℕ) : DensePoly α := cshift i [CField.one]
...
/-- The coefficient of `yⁱ` in `p : DensePoly α` (the `α`-entry at index `i`, `CField.zero` past the
end). -/
def afCoeff (p : DensePoly α) (i : ℕ) : α := (p : List α).getD i CField.zero
