...
theorem bareiss_adjugate_nonRad :
    ...
    (cisZero (csub (polyMatGet prod 0 0) d)
      && cisZero (polyMatGet prod 0 1)
      && cisZero (polyMatGet prod 1 0)
      && cisZero (csub (polyMatGet prod 1 1) d)) = true := by native_decide
...
theorem bareiss_adjugate_trig :
    ...
    (List.range 3).all (fun i => (List.range 3).all (fun j =>
      cisZero (csub (polyMatGet prod i j) (if i = j then d else [])))) = true := by native_decide
...
theorem bareiss_solve_nonRad :
    ...
    let lhs := (List.range 2).map (fun i =>
      (List.range 2).foldl (fun acc j => cadd acc (cmul (polyMatGet M i j) (sol.getD j []))) [])
