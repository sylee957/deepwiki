...
def IsRadicalLogTerm (n : ℕ) (ρ : α) (u integrand : RadElem α) : Prop :=
  Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly (radDeriv n ρ u))
    = Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly u)
      * Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly integrand)
...
theorem mk_toPolyG_radLogSum2 (n : ℕ) (ρ : α) (c₁ : α) (u₁ : RadElem α) (c₂ : α) (u₂ : RadElem α) :
    Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly (radLogSum2 n ρ c₁ u₁ c₂ u₂))
...
theorem isRadicalLogTerm_radGen (n : ℕ) (f : α) :
  ...
  have hint : DensePoly.toPoly ([logDerRadicand n f] : RadElem α) = Polynomial.C (CFieldSpec.toK
...
theorem mk_toPolyG_radLogSumNum_nil (n : ℕ) (ρ : α) (cofs : List (RadElem α)) :
    Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly (radLogSumNum n ρ [] cofs)) = 0 := by
  -- `[].zip cofs = []`, so the fold collapses to the seed `radZero = []` (definitional)
  show Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly (radZero : RadElem α)) = 0
  show Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly ([] : RadElem α)) = 0
...
theorem toPolyG_cAlgResidueResultant_eq_of_eval (D rho g0 g1 : DensePoly α)
  ...
  have hlen : pts.length = 2 * DensePoly.cdeg D + 1 + 1 := by
  ...
  · -- `degree R < #nodes`: `2·deg D + 2 = #nodes` (`cdeg D = (toPoly D).natDegree`)
    rw [Finset.card_range, Nat.cast_withBot]
    have hcd : DensePoly.cdeg D = (DensePoly.toPoly D).natDegree := DensePoly.cdegG_eq_natDegree D
    have hcard : (2 * DensePoly.cdeg D + 1 + 1 : ℕ) = (2 * (DensePoly.toPoly D).natDegree + 2 : ℕ) := by
      rw [hcd]
    rw [hcard]
    exact hRdeg
...
theorem mk_toPolyG_radLogSumNum_eq_sum (n : ℕ) (ρ : α) (args : List (α × RadElem α))
  ...
  rw [show (radZero : RadElem α) = ([] : RadElem α) from rfl, DensePoly.toPolyG_nil, map_zero, zero_add,
    hterms, List.map_map]
  rfl
...
def IsAlgebraicIntegral (n : ℕ) (ρ : α) (f v commonDenomQ : RadElem α)
    (args : List (α × RadElem α)) (cofs : List (RadElem α)) : Prop :=
  Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly (radMul n ρ (radDeriv n ρ v) commonDenomQ))
    + Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly (radLogSumNum n ρ args cofs))
  = Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly (radMul n ρ f commonDenomQ))
...
theorem isAlgebraicIntegral_of_parts (n : ℕ) (ρ : α)
    ...
    (hrat : Ideal.Quotient.mk (radIdeal n ρ)
          (DensePoly.toPoly (radMul n ρ (radDeriv n ρ v) commonDenomQ))
        = Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly (radMul n ρ ratPart commonDenomQ)))
    (hlog : IsRadicalLogIntegral n ρ logPart commonDenomQ args cofs)
    (hsplit : Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly (radMul n ρ ratPart commonDenomQ))
        + Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly (radMul n ρ logPart commonDenomQ))
      = Ideal.Quotient.mk (radIdeal n ρ) (DensePoly.toPoly (radMul n ρ f commonDenomQ))) :
...
#print axioms DensePoly.toPolyG_cAlgResidueNorm

-- Compute bridge: the engine's norm-resultant is the abstract `Polynomial.resultant` under `toK`:
#print axioms DensePoly.toK_cresultantG_cAlgResidueNorm
...
#print axioms DensePoly.toPolyG_cAlgResidueResultant_eq_of_eval
