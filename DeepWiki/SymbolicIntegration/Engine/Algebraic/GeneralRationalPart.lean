...
/-- The rational part of `∫ y dx`: `v = (2/5)·x·y` as `RadElem (QFunNZ ℚ)` `[0, (2/5)x]`. -/
def gcuspVY : RadElem (QFunNZ ℚ) := [CField.zero, qxOfNum [0, 2/5]]

/-- The integrand `y = [0, 1]` as `RadElem (QFunNZ ℚ)`. -/
def gcuspY : RadElem (QFunNZ ℚ) := [CField.zero, CField.one]
...
/-- The driver-produced rational part `v = vNum/y` lifted to `RadElem (QFunNZ ℚ)` `[0, vNum/ρ]`; with
`vNum = (2/5)x⁴`, `ρ = x³` this is `[0, (2/5)x]`. -/
def gcuspVYlift : RadElem (QFunNZ ℚ) :=
  [CField.zero, CField.div (qxOfNum gcuspYRun.2) gcuspRho]

/-- The integrand's rational part `(C − Crem)/y` lifted to `RadElem (QFunNZ ℚ)` `[0, (C − Crem)/ρ]`; with
`C = x³`, `Crem = 0` this is `[0, 1] = y`. -/
def gcuspYRatLift : RadElem (QFunNZ ℚ) :=
...
/-- The rational part of `∫ x·y dx`: `v = (2/7)·x²·y` as `RadElem (QFunNZ ℚ)` `[0, (2/7)x²]`. -/
def gcuspVXY : RadElem (QFunNZ ℚ) := [CField.zero, qxOfNum [0, 0, 2/7]]

/-- The integrand `x·y = [0, x]` as `RadElem (QFunNZ ℚ)`. -/
def gcuspXY : RadElem (QFunNZ ℚ) := [CField.zero, qxOfNum [0, 1]]
...
/-- The driver-produced rational part `v = vNum/y` for `∫ x·y dx`, lifted to `RadElem (QFunNZ ℚ)`:
`[0, vNum/ρ] = [0, (2/7)x²]`. -/
def gcuspVXYlift : RadElem (QFunNZ ℚ) :=
  [CField.zero, CField.div (qxOfNum gcuspXYRun.2) gcuspRho]

/-- The integrand's rational part `(x⁴ − Crem)/y` lifted to `RadElem (QFunNZ ℚ)` `[0, (x⁴ − Crem)/ρ]`;
with `Crem = 0` this is `[0, x] = x·y`. -/
def gcuspXYRatLift : RadElem (QFunNZ ℚ) :=
