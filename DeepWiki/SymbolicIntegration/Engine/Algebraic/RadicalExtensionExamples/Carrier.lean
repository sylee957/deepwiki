...
`F = CFrac ℚ ≅ ℚ(x)`, `n = 2`, `f = x³+1`: checks `y·y = f` and `D(y) = (3x²/(2(x³+1)))·y`. -/

open RadElem

/-- `y·y = f` over `ℚ(x)`: `y = √(x³+1)` squared in `(CFrac ℚ)[y]/(y² − (x³+1))` folds to `f = x³+1`. -/
theorem radGen_sq_eq_radicand :
    DensePoly.cisZero (DensePoly.csub (radMul 2 radicandX3p1 (radGen : RadElem (CFrac ℚ)) radGen) [radicandX3p1])
...
theorem radDeriv_radOne_eq_zero :
    DensePoly.cisZero (radDeriv 2 radicandX3p1 (radOne : RadElem (CFrac ℚ))) = true := by native_decide

/-- Ring sanity `y·1 = y` over `ℚ(x)`: `radMul` with `radOne` is the identity. -/
theorem radMul_radOne_eq :
    DensePoly.cisZero (DensePoly.csub (radMul 2 radicandX3p1 (radGen : RadElem (CFrac ℚ)) radOne) radGen)
      = true := by native_decide
...
theorem radProj_one_radGen :
    DensePoly.cisZero (DensePoly.csub (radProj 1 (radGen : RadElem (CFrac ℚ))) radGen) = true := by native_decide

/-- `T₀(y) = 0`: the projection onto the constant power kills `y`. -/
theorem radProj_zero_radGen :
    DensePoly.cisZero (radProj 0 (radGen : RadElem (CFrac ℚ))) = true := by native_decide

/-- `T₁(1) = 0`: the projection onto the `y`-power kills the constant `1`. -/
theorem radProj_one_radOne :
    DensePoly.cisZero (radProj 1 (radOne : RadElem (CFrac ℚ))) = true := by native_decide

/-- A mixed element `g = (x³+1) + 3x²·y ∈ ℚ(x)[y]/(y²−(x³+1))` (`g₀ = f`, `g₁ = f'`), test integrand for
the `Tᵢ` decoupling. -/
def mixedElem : RadElem (CFrac ℚ) := [radicandX3p1, radicandDeriv]
