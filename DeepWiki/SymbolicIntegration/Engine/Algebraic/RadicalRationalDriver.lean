...
multiplicity `k ≥ 2` solve the cofactor `B = CPoly.radCase2Cofactor` (`h = ρ/W`), form the residual
`D = CPoly.radCase2Residual`, accumulate `B·ρ·W^{k0−k}` into `vNum`, recurse on `−D` at `k−1`; bottom at
...
`deg C ≥ deg f`, cancel the leading term of `C` with `B = CPoly.radCase3Cofactor f g C`, form the residual
`D = CPoly.radCase3Residual`, accumulate `B·f` into `vNum`, recurse on `−D`; bottom at `deg C < deg f`. `der`
