...
over the contribution list: `radDeriv` distributes over `foldl DensePoly.cadd` (`toPolyG_radDeriv_foldl_cadd`)
...
theorem sum_radDeriv_telescope (n : ℕ) (f : α) :
    ∀ (L₀ : RadElem α) (rest : List (RadElem α)) (cs : List (RadElem α)),
      List.Forall₂ (fun c p => DensePoly.toPoly (radDeriv n f c)
            = DensePoly.toPoly p.1 - DensePoly.toPoly p.2)
          cs ((L₀ :: rest).zip rest) →
      (cs.map (fun c => DensePoly.toPoly (radDeriv n f c))).sum
        = DensePoly.toPoly L₀ - DensePoly.toPoly (rest.getLastD L₀) := by
  intro L₀ rest
...
leftovers, then `v = cs.foldl DensePoly.cadd radZero` satisfies
`toPoly (radDeriv n f v) + toPoly (final leftover) = toPoly (original integrand)` in `K[X]`. Composes
`toPolyG_radDeriv_foldl_cadd` and `sum_radDeriv_telescope`. -/
theorem radReduceRationalTelescope (n : ℕ) (f : α) (L₀ : RadElem α) (rest cs : List (RadElem α))
    (hstep : List.Forall₂ (fun c p => DensePoly.toPoly (radDeriv n f c)
          = DensePoly.toPoly p.1 - DensePoly.toPoly p.2)
        cs ((L₀ :: rest).zip rest)) :
    DensePoly.toPoly (radDeriv n f (cs.foldl DensePoly.cadd radZero))
        + DensePoly.toPoly (rest.getLastD L₀)
      = DensePoly.toPoly L₀ := by
  rw [toPolyG_radDeriv_foldl_cadd, toPolyG_radDeriv_radZero, zero_add,
...
accumulates its antiderivative `v` as the `DensePoly.cadd`-fold of pure-`y` step contributions `[0, cBᵢ]`, while
...
single-step Case identity in the genuine field), then the antiderivative `v` assembled as the `DensePoly.cadd`-fold
...
theorem radDeriv_foldl_cadd_zero_cons_telescope (n : ℕ) (f : α)
    ...
    DensePoly.toPoly (radDeriv n f
          ((cBs.map (fun cB => ([CCommRing.zero, cB] : RadElem α))).foldl DensePoly.cadd radZero))
...
(3x²/(2(x³+1)))·√(x³+1)` over `α = DenseFrac ℚ ≅ ℚ(x)`, `n = 2`, `f = x³+1` — now follows from the **general
theorem** `RadElem.toPolyG_radDeriv_radGen` specialized to that base, *without* `native_decide`. The
abstract `toPoly`-equality in `K[X]` (`K = CFieldSpec.K (DenseFrac ℚ)`) is the faithful field-level
...
3x²/(2(x³+1))]` in `K[X]`, `K = CFieldSpec.K (DenseFrac ℚ)`. This strengthens the engine's
...
theorem radDeriv_radGen_sound_qx :
    DensePoly.toPoly (radDeriv 2 radicandX3p1 (radGen : RadElem (DenseFrac ℚ)))
      = DensePoly.toPoly ([CCommRing.zero, radicandLogDer] : RadElem (DenseFrac ℚ)) := by
  rw [toPolyG_radDeriv_radGen]
  rfl

/-! ### Bridge (i): the `CFrac.ofPoly : DensePoly ℚ → DenseFrac ℚ` lift commutes with the derivation

The literal `radIntegrateRational` over `α = DenseFrac ℚ` builds its base-field coefficients by
`CFrac.ofPoly : DensePoly ℚ → DenseFrac ℚ` (`p ↦ ⟨(p, [1]), _⟩`, a polynomial over denominator `1`). Discharging
...
With the three `DenseFrac ℚ` bridges in hand, the per-step `K`-equation precondition of
...
theorem toK_step_ofPoly_iff (n : ℕ) (ρ B C C' : DensePoly ℚ)
    ...
    CFieldSpec.toK (CCommRing.add (CDiffField.cderiv (CFrac.ofPoly B : DenseFrac ℚ))
          (CCommRing.mul (CFrac.ofPoly B : DenseFrac ℚ)
            (logDerRadicand n (CFrac.ofPoly ρ : DenseFrac ℚ))))
        = CFieldSpec.toK (CFrac.ofPoly C : DenseFrac ℚ)
          - CFieldSpec.toK (CFrac.ofPoly C' : DenseFrac ℚ)
...
theorem radDeriv_foldl_cadd_ofPoly_telescope (n : ℕ) (ρ : DensePoly ℚ) (Bpolys Cpolys : List (DensePoly ℚ))
    ...
    (hpoly : ∀ i : ℕ, (hi : i < Bpolys.length) →
      (n : (CFieldSpec.K ℚ)[X]) * DensePoly.toPoly ρ * derivative (DensePoly.toPoly (Bpolys.get ⟨i, hi⟩))
          + DensePoly.toPoly (Bpolys.get ⟨i, hi⟩) * derivative (DensePoly.toPoly ρ)
        = (n : (CFieldSpec.K ℚ)[X]) * DensePoly.toPoly ρ
            * (DensePoly.toPoly (Cpolys.get ⟨i, by omega⟩)
              - DensePoly.toPoly (Cpolys.get ⟨i + 1, by omega⟩))) :
    DensePoly.toPoly (radDeriv n (CFrac.ofPoly ρ)
          (((Bpolys.map CFrac.ofPoly).map (fun cB => ([CCommRing.zero, cB] : RadElem (DenseFrac ℚ)))).foldl
            DensePoly.cadd radZero))
        + DensePoly.toPoly
            ([CCommRing.zero, (Cpolys.map CFrac.ofPoly).getLastD CCommRing.zero] : RadElem (DenseFrac ℚ))
      = DensePoly.toPoly ([CCommRing.zero, (Cpolys.map CFrac.ofPoly).headD CCommRing.zero] : RadElem (DenseFrac ℚ)) := by
  ...
  have hlen' : (Bpolys.map (fun p => (CFrac.ofPoly p : DenseFrac ℚ))).length + 1 =
      (Cpolys.map (fun p => (CFrac.ofPoly p : DenseFrac ℚ))).length := by
    rw [List.length_map, List.length_map]; omega
  have hkey := radDeriv_foldl_cadd_zero_cons_telescope (α := DenseFrac ℚ) n
    (CFrac.ofPoly ρ : DenseFrac ℚ)
    (Bpolys.map (fun p => (CFrac.ofPoly p : DenseFrac ℚ)))
    (Cpolys.map (fun p => (CFrac.ofPoly p : DenseFrac ℚ))) hlen' ?_
  · simpa using hkey
  · -- each step's `K`-equation from the cleared polynomial identity, via `toK_step_ofPoly_iff`
    ...
    rw [show (Bpolys.map (fun p => (CFrac.ofPoly p : DenseFrac ℚ))).get ⟨i, hi⟩ =
          (CFrac.ofPoly (Bpolys.get ⟨i, hiB⟩) : DenseFrac ℚ) from by
        simp [List.get_eq_getElem, List.getElem_map],
      show (Cpolys.map (fun p => (CFrac.ofPoly p : DenseFrac ℚ))).get ⟨i, by omega⟩ =
          (CFrac.ofPoly (Cpolys.get ⟨i, hiC⟩) : DenseFrac ℚ) from by
        simp [List.get_eq_getElem, List.getElem_map],
      show (Cpolys.map (fun p => (CFrac.ofPoly p : DenseFrac ℚ))).get ⟨i + 1, by omega⟩ =
          (CFrac.ofPoly (Cpolys.get ⟨i + 1, hiC1⟩) : DenseFrac ℚ) from by
        simp [List.get_eq_getElem, List.getElem_map]]
...
theorem isRadicalRationalIntegral_div_ofPoly_iff (n : ℕ) (N M ρ : DensePoly ℚ)
    ...
    IsRadicalRationalIntegral n [CFrac.ofPoly ρ]
        ...
        ([CCommRing.zero, CField.div (CFrac.ofPoly N) (CFrac.ofPoly ρ)] : RadElem (DenseFrac ℚ))
  ...
  have hden : (n : RatFunc (CFieldSpec.K ℚ)) * CFrac.am ℚ (DensePoly.toPoly ρ) ≠ 0 :=
    mul_ne_zero hn hρK
  constructor
  · intro h
    apply hinj
...
theorem clearedKX2_of_cisZeroG (N M ρ : DensePoly ℚ)
    (hcheck : DensePoly.cisZero
        (DensePoly.csub (DensePoly.csub (DensePoly.cmul (DensePoly.cscale (2 : ℚ) ρ) (DensePoly.cderiv N))
          (DensePoly.cmul N (DensePoly.cderiv ρ)))
          (DensePoly.cmul (DensePoly.cscale (2 : ℚ) ρ) M)) = true) :
    (2 : (CFieldSpec.K ℚ)[X]) * (DensePoly.toPoly ρ * derivative (DensePoly.toPoly N)
            - DensePoly.toPoly N * derivative (DensePoly.toPoly ρ))
          + DensePoly.toPoly N * derivative (DensePoly.toPoly ρ)
      = (2 : (CFieldSpec.K ℚ)[X]) * DensePoly.toPoly ρ * DensePoly.toPoly M := by
  rw [DensePoly.cisZeroG_iff] at hcheck
...
theorem toPolyG_c3itRho_ne_zero : DensePoly.toPoly (c3itRho : DensePoly ℚ) ≠ 0 := by
  intro h
  have hc : (DensePoly.toPoly (c3itRho : DensePoly ℚ)).coeff 0 = 0 := by rw [h, Polynomial.coeff_zero]
  rw [show (c3itRho : DensePoly ℚ) = [1, 0, 0, 1] from rfl] at hc
  simp only [DensePoly.toPolyG_cons, DensePoly.toPolyG_nil, Polynomial.coeff_add, Polynomial.coeff_C_zero,
...
/-- **★ The `DensePoly.cisZero` engine-test form of the named run's soundness**, abstractly — `DensePoly.cisZero (DensePoly.csub
...
theorem cisZero_radDeriv_c3itVlift
    (hcheck : DensePoly.cisZero
        (DensePoly.csub (DensePoly.csub (DensePoly.cmul (DensePoly.cscale (2 : ℚ) c3itRho) (DensePoly.cderiv c3itRun.2))
          (DensePoly.cmul c3itRun.2 (DensePoly.cderiv c3itRho)))
          (DensePoly.cmul (DensePoly.cscale (2 : ℚ) c3itRho) (DensePoly.csub c3itC c3itRun.1))) = true) :
    DensePoly.cisZero (DensePoly.csub (radDeriv 2 c3itRhoQx c3itVlift) c3itRatLift) = true := by
...
`radDeriv_foldl_cadd_zero_cons_telescope`:
...
> antiderivative `v = (cBs.map (·↦[0,·])).foldl DensePoly.cadd radZero` satisfies
...
2. **★ The fuel-recursion telescoping invariant** — `radReduceRationalTelescope` (built from
   `toPolyG_radDeriv_foldl_cadd` + `sum_radDeriv_telescope`): `radDeriv` distributes over the accumulator
   `foldl DensePoly.cadd` and the per-step contributions telescope to the endpoints. This is the genuinely-new
...
`D(cBᵢ) + cBᵢ·ℓ = cCᵢ − cCᵢ₊₁`, is, for the literal `radIntegrateRational` over `α = DenseFrac ℚ`, each
...
`RatFunc` reasoning). Composed: **`isRadicalRationalIntegral_c3itRun`** / **`cisZero_radDeriv_c3itVlift`**
...
soundness** `radDeriv(assembled v) = integrand − final-leftover`, the three `DenseFrac ℚ` bridges
...
#print axioms cisZero_radDeriv_c3itVlift
