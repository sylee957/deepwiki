...
theorem toPolyG_radDerivFrom (ℓ : α) (k : ℕ) (p : RadElem α) :
    DensePoly.toPoly (radDerivFrom ℓ k p)
      = Differential.mapCoeffs (DensePoly.toPoly p)
        + Polynomial.C (CFieldSpec.toK ℓ)
          * (X * Polynomial.derivative (DensePoly.toPoly p)
              + (k : (CFieldSpec.K α)[X]) * DensePoly.toPoly p) := by
  ...
  | cons a as ih =>
    ...
    have hmc : Differential.mapCoeffs (Polynomial.C (CFieldSpec.toK a) + X * DensePoly.toPoly as)
        = Polynomial.C (Differential.deriv (CFieldSpec.toK a))
          + X * Differential.mapCoeffs (DensePoly.toPoly as) := by
      rw [map_add, Differential.mapCoeffs_C, Derivation.leibniz, Differential.mapCoeffs_X, smul_zero,
        add_zero, smul_eq_mul]
    have hder : Polynomial.derivative (Polynomial.C (CFieldSpec.toK a) + X * DensePoly.toPoly as)
        = DensePoly.toPoly as + X * Polynomial.derivative (DensePoly.toPoly as) := by
...
theorem toPolyG_radDeriv (n : ℕ) (f : α) (p : RadElem α) :
    DensePoly.toPoly (radDeriv n f p)
      = Differential.implicitDeriv
          (Polynomial.C (CFieldSpec.toK (logDerRadicand n f)) * X) (DensePoly.toPoly p) := by
...
theorem toPolyG_radDeriv_linear (n : ℕ) (f a₀ a₁ : α) :
  ...
  have hv : DensePoly.toPoly ([a₀, a₁] : RadElem α)
    ...
    rw [DensePoly.toPolyG_cons, DensePoly.toPolyG_cons, DensePoly.toPolyG_nil, mul_zero, add_zero]
...
@[denote] theorem toPolyG_append (p q : RadElem α) :
    DensePoly.toPoly (p ++ q)
      = DensePoly.toPoly p + X ^ (p : List α).length * DensePoly.toPoly q := by
  induction p with
  | nil => simp
  | cons a as ih =>
    show DensePoly.toPoly (a :: (as ++ q)) = _
    rw [DensePoly.toPolyG_cons, ih, DensePoly.toPolyG_cons]
...
theorem mk_toPolyG_radReduce_step (n : ℕ) (f : α) (q : RadElem α)
  ...
  have htop : DensePoly.toPoly q
      = DensePoly.toPoly (q : List α).dropLast + X ^ m * Polynomial.C (CFieldSpec.toK am) := by
    conv_lhs => rw [← hsplit]
    rw [toPolyG_append, DensePoly.toPolyG_cons, DensePoly.toPolyG_nil, mul_zero, add_zero, ← hmdef]
    simp only [toR_eq_toK]
...
theorem mk_toPolyG_radReduce (n : ℕ) (f : α) (fuel : ℕ) (p : RadElem α) :
    Ideal.Quotient.mk (radIdeal n f) (DensePoly.toPoly (radReduce n f fuel p))
      = Ideal.Quotient.mk (radIdeal n f) (DensePoly.toPoly p) := by
  induction fuel generalizing p with
  | zero => rw [radReduce]
  | succ fuel ih =>
    rw [radReduce]
    by_cases hlen : (DensePoly.cnorm p : List α).length ≤ n
    · simp only [hlen, if_true, DensePoly.toPolyG_cnormG]
    · -- the loop folds the top coefficient and recurses; `ih` + one step + `toPolyG_cnormG`
      have hlt : n < (DensePoly.cnorm p : List α).length := Nat.lt_of_not_le hlen
      have hqne : (DensePoly.cnorm p : List α) ≠ [] := by
...
theorem mk_toPolyG_radMul (n : ℕ) (f : α) (a b : RadElem α) :
    Ideal.Quotient.mk (radIdeal n f) (DensePoly.toPoly (radMul n f a b))
      = Ideal.Quotient.mk (radIdeal n f) (DensePoly.toPoly a)
        * Ideal.Quotient.mk (radIdeal n f) (DensePoly.toPoly b) := by
...
theorem mk_toPolyG_radDeriv_radMul (n : ℕ) (f : α)
    ...
    Ideal.Quotient.mk (radIdeal n f) (DensePoly.toPoly (radDeriv n f (radMul n f a b)))
      = Ideal.Quotient.mk (radIdeal n f) (DensePoly.toPoly (radMul n f (radDeriv n f a) b))
        + Ideal.Quotient.mk (radIdeal n f) (DensePoly.toPoly (radMul n f a (radDeriv n f b))) := by
  -- abbreviations
  set ℓX := Polynomial.C (CFieldSpec.toK (logDerRadicand n f)) * X with hℓX
  set A := DensePoly.toPoly a with hA
  set B := DensePoly.toPoly b with hB
...
@[denote] theorem toPolyG_radDeriv_radZero (n : ℕ) (f : α) :
    DensePoly.toPoly (radDeriv n f (radZero : RadElem α)) = 0 := by
  rw [toPolyG_radDeriv, show (radZero : RadElem α) = [] from rfl, DensePoly.toPolyG_nil, map_zero]
