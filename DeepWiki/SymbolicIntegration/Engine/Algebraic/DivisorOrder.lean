...
def cantorOrderAux (fuel : ℕ) (ρ : DensePoly α) (g : ℕ)
...
def cantorOrder (fuel : ℕ) (ρ : DensePoly α) (g : ℕ) (D : MumfordDivisor α) : Option ℕ :=
...
def orderModP (p fuel : ℕ) [Fact p.Prime] (ρ : DensePoly ℚ) (g : ℕ) (D : MumfordDivisor ℚ) :
    Option ℕ :=
...
def isTorsionDivisor (p : ℕ) [Fact p.Prime] (ρ : DensePoly ℚ) (g : ℕ) (D : MumfordDivisor ℚ) :
    Option ℕ :=
...
def elementarityViaTorsion (p : ℕ) [Fact p.Prime] (ρ : DensePoly ℚ) (g : ℕ) (D : MumfordDivisor ℚ) :
    Bool :=
...
def rhoX3p1Mod5 : DensePoly (ZMod 5) := polyToZMod 5 hypRhoX3p1
...
def hypRhoX3m2 : DensePoly ℚ := [-2, 0, 0, 1]
