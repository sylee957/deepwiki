...
def radicandX5mXm1 : QFunNZ ℚ := qxOfNum [-1, -1, 0, 0, 0, 1]
...
theorem not_isSquare_radicandX5mXm1 :
    ∀ b : RatFunc ℚ, b ^ 2 ≠ CFieldSpec.toK (radicandX5mXm1 : QFunNZ ℚ) := by
...
theorem irreducible_radX5mXm1 :
    Irreducible (X ^ 2 - C (CFieldSpec.toK (radicandX5mXm1 : QFunNZ ℚ))) :=
...
instance fact_irreducible_radX5mXm1 :
    Fact (Irreducible (X ^ 2 - C (CFieldSpec.toK (radicandX5mXm1 : QFunNZ ℚ)))) :=
  ⟨irreducible_radX5mXm1⟩

/-- The radical field `ℚ(x)[√(x⁵−x−1)] = RadExt (QFunNZ ℚ) 2 (x⁵−x−1)`. -/
abbrev RadX5 : Type := RadExt (QFunNZ ℚ) 2 radicandX5mXm1
...
base. -/
theorem cfield_qfunNZG_radX5 : Nonempty (CField (QFunNZ RadX5)) := ⟨inferInstance⟩

/-- **The mixed tower over `√(x⁵−x−1)` is a `CDiffField`**. -/
theorem cdiffField_qfunNZG_radX5 : Nonempty (CDiffField (QFunNZ RadX5)) := ⟨inferInstance⟩
...
`√(x⁵−x−1)`. -/
def radX5LogDer : QFunNZ ℚ := logDerRadicand 2 radicandX5mXm1

/-- The `RadX5[t]`-polynomial `t² = [0,0,1]`. -/
def radX5T2sq : CPoly RadX5 := [CField.zero, CField.zero, CField.one]

/-- The `RadX5[t]`-polynomial `2·t² = [0,0,2]`, the expected `D(t²)` for `t = eˣ`. -/
def radX5TwoT2sq : CPoly RadX5 := [CField.zero, CField.zero, CField.add CField.one CField.one]

/-- The monomial-derivative datum `Dt = t = [0,1]` over `RadX5` (`t = eˣ`). -/
def radX5DtExp : CPoly RadX5 := [CField.zero, CField.one]
...
def radX5GenTDeriv : CPoly RadX5 :=
