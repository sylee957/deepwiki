/-! # Relational refinement kernel

A heterogeneous relation connects concrete and abstract carriers, while the respectful arrow lifts
relations to functions. `Refines.app` composes operation witnesses, providing the kernel used by the
separate transfer-automation layer. -/
...
/-- The respectful arrow relates functions that send related inputs to related outputs. -/
...
/-- A related function applied to a related argument yields related results. -/
...
/-- `Subsumes R S`: the finer relation `R` implies the coarser `S`, permitting witness weakening. -/
