...
A general, topic-agnostic metaprogram that synthesizes the abstract denotation of a computable
expression, driven by the `denote` simp set. The CoqEAL `⟹`/`refines_apply` pure-typeclass kernel
cannot decompose in Lean (the composition instance needs a metavariable function-head unification
`?g ?b =?= cop x y` that TC will not solve), so — like Trocq's Coq-Elpi plugin — transfer is a
metaprogram. `simp only [denote]` performs the recursive decomposition; this layer orchestrates it to
hand back the abstract term as *first-class data* (the proof's RHS is computed, not hand-written),
which solves the `= _` hole problem that `simp`/TC hit.

* `transfer% e` — a term producing a proof of `e = <abstract>`, with `<abstract>` computed.
* `transfer` (tactic) — whole-goal: rewrite every denotation application to its abstract form.
-/
