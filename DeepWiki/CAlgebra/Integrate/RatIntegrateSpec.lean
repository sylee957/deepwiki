...
antiderivative `rational + ∫poly + ∑ᵢ ∑_{Qᵢ(α)=0} α · log Sᵢ(α, x)`. -/
noncomputable def RatIntegral.deriv (res : RatIntegral R) : RatFunc R :=
  RatFunc.deriv (DenseFrac.toRatFunc res.rational) + toRatFuncHom (res.poly′)
    + res.logs.deriv
...
theorem ratIntegrate_sound (f : DenseFrac R) :
  ...
  show RatFunc.deriv (DenseFrac.toRatFunc (hermiteReduce f).rational)
