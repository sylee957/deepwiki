...
/-! # Per-flow end-to-end delay / backlog in a stable static-priority network (capstone)
Connecting the two halves of the theory: a locally stable static-priority network `Traj` (the
stability side — every flow's per-hop bursts are bounded by `allBound`) yields *finite end-to-end
delay and backlog* for every flow (the performance side — the SP-PMOO end-to-end rate-latency bound).
The bridge is `concatComp_of_chain`: flow `i`'s per-hop trajectory `proc` realizes the SP-PMOO
`concatComp` chain, whose cross-traffic affine bounds come from `allBound` and whose stability
`rᵢ < R^(h) − ρ^(h)` comes from the network's local stability. The shared assembly is
`spPmooEndToEndSetup`; delay and backlog are its two corollaries. -/
...
/-- **The SP-PMOO end-to-end setup for a stable network**: flow `i`'s per-hop trajectory realizes the
SP-PMOO `concatComp` chain along its path, with affine cross-traffic (`hcross`), per-server stability
(`hstabρ`/`hr`/`hRpos`), and a token-bucket ingress (`harr`) — everything `delay_le_spPmoo_rateLatency`
/ `backlog_le_spPmoo_rateLatency` need. Cross-traffic bounds come from `allBound`; the per-server
stability `rᵢ < R^(h) − ρ^(h)` from local stability `∑_{Fl h} r < R^(h)`. -/
theorem spPmooEndToEndSetup (t : Traj κ ι) (i : ι) (hne : t.net.paths i ≠ []) :
    ∃ (α : κ → ι → ℝ≥0 → ℝ≥0) (ρ bc : κ → ℝ≥0) (head : κ) (tail : List κ),
      t.net.paths i = head :: tail ∧
      (∀ h, (fun v => ∑ j ∈ Finset.univ.filter (fun j => j < i), α h j v)
        = fun v => ρ h * v + bc h) ∧
      (∀ h, ρ h < t.R h) ∧
      concatComp (fun h => residualServer (fun As Ds => t.S h As Ds ∧
...
/-- **Per-flow end-to-end delay in a stable static-priority network**: every flow with a nonempty
path has a *finite end-to-end delay* (ingress to egress), via `spPmooEndToEndSetup` +
`delay_le_spPmoo_rateLatency`. -/
theorem isFlowEndToEndDelayBounded (t : Traj κ ι) (i : ι) (hne : t.net.paths i ≠ []) :
  ...
  obtain ⟨α, ρ, bc, head, tail, hpath, hcross, hstabρ, hp, harr, hRpos, hr⟩ :=
    t.spPmooEndToEndSetup i hne
  rw [hpath]
  exact ⟨_, delay_le_spPmoo_rateLatency (S := t.S) (R := t.R) (T := t.T) (α := α) (i := i)
    (ρ := ρ) (bc := bc) (r := t.r i) (b := t.b i) t.hcaus
    (fun h => isLeftContinuous_of_continuous _ (rateLatency_continuous (t.R h) (t.T h)))
    t.hβ t.hSP hcross hstabρ hp harr hRpos hr⟩

/-- **Per-flow end-to-end backlog in a stable static-priority network**: every flow with a nonempty
path has a *finite end-to-end backlog*, via `spPmooEndToEndSetup` + `backlog_le_spPmoo_rateLatency`. -/
theorem isFlowEndToEndBacklogBounded (t : Traj κ ι) (i : ι) (hne : t.net.paths i ≠ []) :
  ...
  obtain ⟨α, ρ, bc, head, tail, hpath, hcross, hstabρ, hp, harr, _hRpos, hr⟩ :=
    t.spPmooEndToEndSetup i hne
  rw [hpath]
  exact ⟨_, backlog_le_spPmoo_rateLatency (S := t.S) (R := t.R) (T := t.T) (α := α) (i := i)
    (ρ := ρ) (bc := bc) (r := t.r i) (b := t.b i) t.hcaus
    (fun h => isLeftContinuous_of_continuous _ (rateLatency_continuous (t.R h) (t.T h)))
