...
The concrete `DensePoly α := List α` engine and the representation-generic `CPolyRepr` interface, at the
dense `List` instance, denote **identically**: `CPolyRepr.toPoly p = DensePoly.toPoly p`. Since every engine
...
`DensePoly.c* → CPolyRepr.*` incrementally, gate-green.
...
theorem toPoly_list_eq (p : List α) : CPolyRepr.toPoly p = DensePoly.toPoly p := by
  apply Polynomial.ext; intro k
  rw [CPolyRepr.coeff_toPoly, DensePoly.toPolyG_coeff]
  rfl

/-- The generic `add` agrees with the engine `cadd` under the denotation — so a call site may swap
`DensePoly.cadd` for `CPolyRepr.add` and every `toPoly`-stated theorem still holds. -/
theorem toPoly_add_eq_cadd (p q : List α) :
    DensePoly.toPoly (CPolyRepr.add p q) = DensePoly.toPoly (DensePoly.cadd p q) := by
...
theorem toPoly_mul_eq_cmul (p q : List α) :
    DensePoly.toPoly (CPolyRepr.mul p q) = DensePoly.toPoly (DensePoly.cmul p q) := by
...
theorem toPoly_neg_eq_cneg (p : List α) :
    DensePoly.toPoly (CPolyRepr.neg p) = DensePoly.toPoly (DensePoly.cneg p) := by
  rw [← toPoly_list_eq, CPolyRepr.toPoly_neg, toPoly_list_eq, ← DensePoly.toPolyG_cnegG]
...
theorem cdeg_list_eq (p : List α) : CPolyRepr.cdeg p = DensePoly.cdeg p := by
  rw [CPolyRepr.cdeg_eq_natDegree, toPoly_list_eq, ← DensePoly.cdegG_eq_natDegree]

/-- The interface `clead` agrees with the engine `clead` under `toR` (both are `leadingCoeff`). -/
theorem toR_clead_list_eq (p : List α) :
    CRingSpec.toR (CPolyRepr.clead p) = CRingSpec.toR (DensePoly.clead p) := by
  rw [CPolyRepr.toR_clead_eq_leadingCoeff, toPoly_list_eq, ← DensePoly.toR_cleadG_eq_leadingCoeff]

/-- The interface `cisZero` equals the engine `cisZero` at `List` (both decide `toPoly p = 0`). -/
theorem cisZero_list_eq (p : List α) : CPolyRepr.cisZero p = DensePoly.cisZero p := by
  rw [← Bool.coe_iff_coe, CPolyRepr.cisZero_iff, toPoly_list_eq, ← DensePoly.cisZeroG_iff]
...
theorem toPoly_cnorm_eq_cnorm (p : List α) :
    DensePoly.toPoly (CPolyRepr.cnorm p) = DensePoly.toPoly (DensePoly.cnorm p) := by
  rw [← toPoly_list_eq, CPolyRepr.toPoly_cnorm, toPoly_list_eq, ← DensePoly.toPolyG_cnormG]
