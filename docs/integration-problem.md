...
## The new-monomial conditions

Each instance is conditional on one hypothesis: that the adjoined monomial is
_genuinely new_ (does not collapse into $F$). Two definitions:

- [`NondegenerateLog`](../DeepWiki/SymbolicIntegration/LiouvilleLogExtension.lean#L365) `u` $:=$
  for every monic irreducible $\pi \in F[X]$, the log-monomial derivation gives
  $D\pi \neq 0$. Meaning: $\log u \notin F$ — no $s \in F$ is an antiderivative of
  $u'/u$ (such an $s$ would make $\pi = X - s$ a monic irreducible with $D\pi = 0$).
- [`NondegenerateExp`](../DeepWiki/SymbolicIntegration/LiouvilleExpExtension.lean#L460) `u` $:=$
  for every nonzero $k \in \mathbb{Z}$ and nonzero $g \in F$,
  $\operatorname{logDeriv} g \neq k\,u'$. Meaning: $\exp(k u) \notin F$ for every
  $k \neq 0$ — the multiplicative analogue.

Both are the Risch _new-monomial condition_, and both are necessary: drop the
condition and the monomial collapses into $F$, making the instance below false.

## The Liouville instances, component by component
...
- `u : F` — the element whose logarithm is adjoined.
- `hnd : NondegenerateLog u` — the new-monomial hypothesis above ($\log u \notin F$).
- `letI := logDifferential u` — equips `RatFunc F` with the derivation $t' = u'/u$
  (the defining equation of $\log u$), making it a differential field.
- `letI := logDifferentialAlgebra u` — records $F \subseteq{}$`RatFunc F` as a
  differential-field _extension_.
- conclusion `IsLiouville F (RatFunc F)` — the extension $F \subseteq F(\log u)$ is
  Liouville: any $f \in F$ with an antiderivative elementary over $F(\log u)$ already
  has the Liouville form $D(v) + \sum_i c_i\,D(u_i)/u_i$ inside $F$.

The exponential instance
[`isLiouville_expExtension_uncond`](../DeepWiki/SymbolicIntegration/ComputableLiouvilleExpBridge.lean#L775)
is the same component by component, with `NondegenerateExp u` in place of `hnd` and
`expDifferential u` fixing $t' = u'\,t$. `IsLiouville` composes across extensions, so
chaining these single steps lifts Liouville's theorem up the whole tower.
