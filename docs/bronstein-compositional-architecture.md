...
3. Connect one-level relative completeness to the recursive tower path. This needs a separate
   ...
   constant. Full monomial completeness still requires proving that this domain covers every coefficient pair
   generated by the stage; that mathematical constant-descent/search theorem remains open.
