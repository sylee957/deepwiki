...
Lean 4. The punchline: the project mechanizes both directions through the single
driver [`cIntegrateGFull`](../DeepWiki/SymbolicIntegration/ComputableTowerRischDE.lean#L681).
_Soundness_ — that every integral the algorithm returns is correct, $D(\int f) = f$,
across all three regimes (rational, exponential, primitive) — and the
...
the tangent case — modulo the honest new-monomial side conditions stated explicitly.
...
and completeness results as they are proved in the library. Throughout, a
declaration name links to its definition in the local source tree.
