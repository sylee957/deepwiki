...
logarithmic derivative `Dw/w` — so the vehicle is `CPoly.paramRischDE [a, η]` directly, not the log-variant
`CPoly.limitedIntegrate`.

## What we already have

- **Base ℚ constant-field linear solver** (`Parametric.lean`): `crref`, `CLinearSolve.nullspaceBasis`, `cConstSolveUniqueQ`.
  **Reusable verbatim at every tower level** — `Const(k(t)) = Const(k) = ℚ` for a monomial `t`, so the linear
  algebra is always over ℚ. (Widely reused already: `cConstSolveUniqueQ` in 6 files, `CLinearSolve.nullspaceBasis` in 4.)
- **Base parametric RDE** `CPoly.paramRischDE (gnums gdens : List (CPolyG ℚ))` — solves `Dp = Σ cᵢ gᵢ` for a
  **polynomial** `p ∈ ℚ[t]`, returning the ℚ-kernel of admissible `(c₁,…,cₘ)`. It clears denominators
  (`CPoly.linearConstraintsQ`) and reads the ℚ-matrix `Mᵢⱼ = coeff(rⱼ, tⁱ)`.
- **Base limited integration** `CPoly.limitedIntegrate` — the §7.2 *log* variant (generators `Dwᵢ/wᵢ`), **polynomial
...
1. **Base is polynomial-only.** `CPoly.paramRischDE`/`CPoly.limitedIntegrate` solve for *polynomial* `p`. A complete base
...
- **Phase 1b — base rational `b`.** Hermite-reduce the rational part before the kernel solve, so `b ∈ ℚ(x)`
  (not just `ℚ[x]`). Extends `CPoly.limitedIntegrate` past its polynomial-only limitation.
