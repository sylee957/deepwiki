...
- Render the book to HTML: render to a **staging dir**, then swap it into
  place — `lake exe generate-book --output _out/.staging` and then
  `rm -rf _out/html-multi && mv _out/.staging/html-multi _out/html-multi`.
  Writes `_out/html-multi/`; `_out/` is gitignored.
- **Do not** render straight into `_out` while something holds it: the
  renderer writes *alongside* any locked output (a running `python3 -m
  http.server`, or a shell whose cwd is inside `_out/html-multi`, pins
  it), producing a duplicate `html-multi 2/` — and `index.html` ends up
  inside *that*, so the main dir looks broken. Rendering to a separate
  staging dir and `mv`-swapping avoids the lock entirely (a `mv` within
  `_out` is a rename, so a live server keeps serving the old inode and
  picks up the new files on its next request — nothing is killed).
- Serve for viewing (Verso needs HTTP, not `file://`, for code hovers),
  from the **repo root** so the server's cwd never pins `html-multi`:
...
`Additivity`, `Closures`, `Continuity`, `RealFunctionClasses`,
